				...
				<table class="table table-bordered table-hover">
					...
					<tbody>
						<?php $selectedId = ""; 
							...
							<tr class="row1">
								<?php $selectedId .= $result->RECEIPT_ID .",";  ?>
								<!--HIDDEN FIELD USED FOR UNCHECK CHECKBOX-->
								<input type="hidden" name="pageVal" id="pageVal"/>
								<?php if(isset($_SESSION['all_users'])) { ?>
									<td><center><input type="checkbox" name="<?php echo $result->RECEIPT_ID; ?>" id="<?php echo $result->RECEIPT_ID; ?>" onchange="GetUnselect('<?php echo $result->RECEIPT_ID; ?>')" class="sel" checked /></center></td>
								<?php } else { ?>
									<td><center><input type="checkbox" name="<?php echo $result->RECEIPT_ID; ?>" id="<?php echo $result->RECEIPT_ID; ?>" onchange="GetUnselect('<?php echo $result->RECEIPT_ID; ?>')" class="sel"/></center></td>
								<?php } ?>
...
<script>
	var NewInput = document.getElementsByClassName("sel");
	<?php if(isset($_SESSION['this_page'])) { ?>
	for(var i = 0; i < NewInput.length; i++){
		$('#'+NewInput[i]).prop('checked') == true;
	}
	<?php } ?>
	...
	function GetUnselect(receiptId) {
		...
		if($('#all_users').prop('checked') == true) {
			...
			if(count == inputs.length) {
				...
				if('<?php echo $countEodRows ?>' == count) {
