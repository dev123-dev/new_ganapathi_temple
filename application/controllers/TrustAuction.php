<?php defined('BASEPATH') OR exit('No direct script access allowed');
	class TrustAuction extends CI_Controller {
		...
		function get_saree_details() { 
			...
			$auctItemList = $this->obj_auction->get_item_details("TRUST_AUCTION_ITEM_LIST",$condition); 
			$condtBidRange = array('IBR_AI_ID' =>$auctItemList[0]->AIL_ITEM_ID,'IBR_AIC_ID' =>$auctItemList[0]->AIL_AIC_ID);
			$bidRange = $this->obj_auction->get_bid_ranges("TRUST_AUCTION_ITEM_BID_RANGE",$condtBidRange);
			$condtDefBidRange = array('IDB_AI_ID' =>$auctItemList[0]->AIL_ITEM_ID,'IDB_AIC_ID' =>$auctItemList[0]->AIL_AIC_ID);
			$defaultBidRange = $this->obj_auction->get_default_bid("TRUST_AUCTION_ITEM_DEFAULT_BID",$condtDefBidRange);
		...
		function add_auction_item_list() {
			...
			$this->db->select()->from('TRUST_AUCTION_ITEM')
			->where(array('TRUST_AUCTION_ITEM.AI_STATUS'=>1));
			
			$query = $this->db->get();
			$data['items'] = $query->result();
			
			$this->db->select()->from('TRUST_AUCTION_ITEM_CATEGORY')
			->where(array('TRUST_AUCTION_ITEM_CATEGORY.AIC_STATUS'=>1));
			$query = $this->db->get();
			...
			if(isset($_SESSION['Add_Auction_Item'])) {
				...
				$this->load->view('trust/auction/add_auction_item');
		...
		function bid_auction_item() {
			...
			$this->db->select('*');
			$this->db->from('TRUST_AUCTION_ITEM_LIST');
			$this->db->where('TRUST_AUCTION_ITEM_LIST.AIL_ITEM_STATUS', 1);
			$this->db->group_by('AIL_ITEM_NAME');
			
			$query = $this->db->get();
			$data['items'] = $query->result();
			
			$this->db->select()->from('TRUST_AUCTION_ITEM_LIST')
			->where(array('TRUST_AUCTION_ITEM_LIST.AIL_ITEM_STATUS'=>1))->order_by('AIL_AIC_ID','asc');
			...
			if(isset($_SESSION['Bid_Auction_Item'])) {
				...
				$this->load->view('header',$data);
				$this->load->view('trust/auction/bid_auction_item');
				$this->load->view('footer_home');
			} else {
				redirect('Home/homePage');
			}
		...
		function issue_auction_receipt_save() {
			...
			// edited by adithya edited the fglhbank column 
			...
			$data_event_receipt = array('TET_RECEIPT_NO' => $receiptFormat,
										...
										'TET_RECEIPT_PRICE' => $this->input->post('bidPrice'),
										'TET_RECEIPT_PAYMENT_METHOD_NOTES' => $this->input->post('bilRefNo'),
			...
			$id = $this->obj_auction->add_auction_receipt_modal('TRUST_AUCTION_RECEIPT',$data_array);
			
			redirect('/TrustAuction/issue_receipt_print/'.$id);
		}
		...
		function issue_auction_receipt($id) {
            ...
            // added by adithya on 5-1-24 end			//laz new ..
			
			$data['fromAllReceipt'] = "1";
			$_SESSION['duplicate'] = "no";
			$data['event'] = $this->obj_events->getEvents();
			$condition = array('BIL_ID' => $id);
			$data['auction_receipt'] = $this->obj_auction->get_auction_receipt('TRUST_BID_ITEM_LIST',$condition);
			
			$this->load->view('header',$data); 
			$this->load->view('trust/auction/issue_auction_receipt');
			$this->load->view('footer_home');
		}
		...
		function issue_receipt_print($id) {
			...
			$data['auction_receipt'] = $this->obj_auction->get_auction_receipt('TRUST_AUCTION_RECEIPT',$condition);
			
			$condition = array('TET_RECEIPT_ID' => $data['auction_receipt'][0]->TET_RECEIPT_ID);
			$data['notes'] = $this->obj_auction->get_auction_receipt('TRUST_EVENT_RECEIPT',$condition);
			...
			$this->load->view('header',$data); 
			$this->load->view('trust/auction/issue_receipt_print');
			$this->load->view('footer_home');
		}
		...
		function SearchAuctionReceipt($start = 0) {
			...
			$data['event'] = $this->obj_events->getEvents();
			$data['auction_receipt'] = $this->obj_auction->get_auction_receipt('TRUST_AUCTION_RECEIPT');
			$data['bid_item_list'] = $this->obj_auction->get_bid_item_list_like('TRUST_BID_ITEM_LIST',$condition,"","",10,$start);
			
			//pagination starts
			$this->load->library('pagination');
			$config['base_url'] = base_url().'TrustAuction/SearchAuctionReceipt';
			$config['total_rows'] = $this->obj_auction->count_rows_bid_item_like('TRUST_BID_ITEM_LIST',$condition);
			$config['per_page'] = 10;
			$config['prev_link'] = '&lt;&lt;';
			$config['next_link'] = '&gt;&gt;';
			...
			$this->load->view('header',$data);
			$this->load->view('trust/auction/issue_auction');
			$this->load->view('footer_home');
		}
		...
		function issue_auction($start = 0) {
			...
			$data['event'] = $this->obj_events->getEvents();
			$data['auction_receipt'] = $this->obj_auction->get_auction_receipt('TRUST_AUCTION_RECEIPT');
			$data['bid_item_list'] = $this->obj_auction->get_bid_item_list('TRUST_BID_ITEM_LIST',"","","",10,$start);
			$data['name'] = "";
			$data['bidNo'] = "";
			unset($_SESSION['name']);
			...
			//pagination starts
			$this->load->library('pagination');
			$config['base_url'] = base_url().'TrustAuction/issue_auction';
			$config['total_rows'] = $this->obj_auction->count_rows_bid_item('TRUST_BID_ITEM_LIST');
			$config['per_page'] = 10;
			$config['prev_link'] = '&lt;&lt;';
			$config['next_link'] = '&gt;&gt;';
			$config['first_tag_open'] = '<li>';
		...
		function checkSareeDate() {
			...
			if($AIC_ID == 1) {
				$this->db->select()->from('TRUST_AUCTION_ITEM_LIST')
				->where(array('TRUST_AUCTION_ITEM_LIST.AIL_EVENT_ID'=>$_POST['event_name'], 'AIL_AIC_ID'=>$AIC_ID,'AIC_SEVA_DATE'=>$_POST['sevadate']));
				$query = $this->db->get();
				$num = $query->num_rows();
		...
		function updateSareeDate() {
			...
			if(isset($_POST['swapDate'])) {
				...
				$this->obj_auction->update_auction_model("TRUST_AUCTION_ITEM_LIST", $condition, $updateActionItem);
		...
		function checkHarivanaExist() {
			...
			if($AI_ID != 2) {
				$this->db->select()->from('TRUST_AUCTION_ITEM_LIST')
				->where(array('TRUST_AUCTION_ITEM_LIST.AIL_EVENT_ID'=>$data['event']['TET_ID'], 'AIL_ITEM_ID'=>$AI_ID));
				$query = $this->db->get();
				$num = $query->num_rows();
			} else if($AIC_ID == 3) {
				$this->db->select()->from('TRUST_AUCTION_ITEM_LIST')
				->where(array('TRUST_AUCTION_ITEM_LIST.AIL_EVENT_ID'=>$data['event']['TET_ID'], 'AIL_AIC_ID'=>$AIC_ID));
				$query = $this->db->get();
				$num = $query->num_rows();
			} else if($AIC_ID == 1) {
				$this->db->select()->from('TRUST_AUCTION_ITEM_LIST')
				->where(array('TRUST_AUCTION_ITEM_LIST.AIL_EVENT_ID'=>$data['event']['TET_ID'], 'AIL_AIC_ID'=>$AIC_ID,'AIC_SEVA_DATE'=>$_POST['sevadate']));
				$query = $this->db->get();
				$num = $query->num_rows();
			}else if($AIC_ID == 4) {
				$this->db->select()->from('TRUST_AUCTION_ITEM_LIST')
		...
		function bid_auction_preview() {
		    ...
		    if(isset($_POST['bidRefNoView'])) {
				...
				$this->db->where($where);
				$this->db->update('TRUST_BID_ITEM_LIST',$data2);
			}
			...
			if(isset($_SESSION['From_Bid_Auction_Item'])){
				...
				$this->db->select()->from('TRUST_BID_REFERENCE_COUNTER')
				->where(array('TRUST_BID_REFERENCE_COUNTER.BRC_ID'=>1));
				$query = $this->db->get();
				$res = $query->first_row();
				...
				$dataInsert = array(
					...
					'BIL_EVENT_NAME'=>$data['event']['TET_NAME'],
					'BIL_EVENT_ID'=>$data['event']['TET_ID'],
					'BIL_NAME'=>$data['name'],
				...
				$insertId = $this->obj_auction->add_auction_modal("TRUST_BID_ITEM_LIST", $dataInsert);
				
				if($insertId != "") {
					$updateActionItem = array('BRC_COUNTER'=>$bidNo);
					$this->obj_auction->update_auction_model("TRUST_BID_REFERENCE_COUNTER", "", $updateActionItem);
			...
			$this->db->select('PRINT_STATUS')->from('TRUST_BID_ITEM_LIST')
			->where(array('TRUST_BID_ITEM_LIST.BID_REF_NO'=>$_SESSION['bidRefNo']));
			$query = $this->db->get();
			...
			$data['templename'] = $templename =$this->obj_shashwath->getTempleDetails();
			$this->load->view('header',$data);
			$this->load->view('trust/auction/bid_auction_preview');
			$this->load->view('footer_home'); 
		}
		...
		function AuctionPrintStatus(){
			...
			$this->db->where($where);
			$this->db->update('TRUST_AUCTION_RECEIPT',$data2);
		}
		...
		function auction_item_details() {
			...
			$addAuctionItems = array(
				...
				"AIL_EVENT_NAME"=>$data['event']['TET_NAME'],
				"AIL_EVENT_ID"=>$data['event']['TET_ID'],
				"ITEM_REF_NO"=>$refNo,
			...
			$insertId = $this->obj_auction->add_auction_modal("TRUST_AUCTION_ITEM_LIST", $addAuctionItems);
			
			if($insertId != "") {
				$condition = array('AI_ID'=>$AI_ID);
				$updateActionItem = array('AI_COUNTER'=>$AI_COUNTER);
				$this->obj_auction->update_auction_model("TRUST_AUCTION_ITEM", $condition, $updateActionItem);
			}
			
			$this->load->view('header',$data);
			$this->load->view('trust/auction/auction_item_details');
			$this->load->view('footer_home');
		}
	}
