...
class EventEOD_Tally extends CI_Controller {
	...
	public function index($start = 0) {
		...
		$config['base_url'] = base_url().'EventEOD_Tally/index';
	...
	function checkPreviousPendingDate() {
		$toDate = $_POST['date'];
		$sql = "SELECT EUC_EOD_DATE as EODDate FROM `event_user_collection` WHERE EUC_EOD_DATE < '".$toDate."' and EUC_IS_DEPOSITED != 1 ";
	...
	function get_change_on_filters($start = 0) {
		...
		$this->db->from("EVENT_USER_COLLECTION")->where($arr)->or_where($or_arr);
		$query = $this->db->get();
		...
		//pagination starts
		$this->load->library('pagination');
		$config['base_url'] = base_url().'EventEOD_Tally/get_change_on_filters';
		$config['total_rows'] = $this->obj_eod->count_get_all_field($fromDate, $toDate);
		$config['per_page'] = 30;
		$config['prev_link'] = '&lt;&lt;';
		$config['next_link'] = '&gt;&gt;';
	...
	function eod_tally_report_excel() {
		...
		for($i = 0; $i < sizeof($res); $i++) {
			...
			$value = "";			
			$value .= '"' . ($i+1) . '"' . "\t";
			$value .= '"' . date('d-m-Y',strtotime($res[$i]->EUC_DATE)) . '"' . "\t";
			$value .= '"' . $res[$i]->OpBal . '"' . "\t";
			
			if($res[$i]->EUC_CASH != NULL)
				$value .= '"'.$res[$i]->EUC_CASH .'"' . "\t";
			else
				$value .= '"'.$res[$i]->EUC_CASH_DEPOSIT .'"' . "\t";
			
			if($res[$i]->EUC_CHEQUE != NULL)
				$value .= '"' . $res[$i]->EUC_CHEQUE . '"' . "\t";
			else 
				$value .= '"' . $res[$i]->EUC_CHEQUE_DEPOSIT . '"' . "\t";
			
			if($res[$i]->EUC_CASH_CHEQUE != NULL)
				$value .= '"' . $res[$i]->EUC_CASH_CHEQUE . '"' . "\t";
			else 
				$value .= '" - "' . "\t";
			
			if($res[$i]->EUC_DEBIT != NULL)
				$value .= '"' . $res[$i]->EUC_DEBIT . '"' . "\t";
			else 
				$value .= '" - "' . "\t";
	...
	function deactivateCheque() {
		...
		$where = array(
			'EUC_ID'=>$EUC_CHEQUE_ID
		);
		$this->db->where($where);
		$this->db->update('EVENT_USER_COLLECTION', $data);
		
		$this->db->delete('EVENT_USER_COLLECTION', array('EUC_ID' => $EUC_ID));
	}
	...
	function eodTallySave() {
		...
		$total = $_POST['total'];
		$bank = $_POST['bank'];
		$ET_RECEIPT_ID = $_POST['ET_RECEIPT_ID'];
		...
		$data = array(
			...
			'EUC_IS_DEPOSITED'=>1,
			'EUC_LEDGER_ID'=>$bank,
			'EUC_DEPOSIT_DATE'=>date('Y-m-d',strtotime($dDate)),
		);
		...
		$sqlFinTrans = "SELECT RECEIPT_ID,VOUCHER_NO FROM `financial_ledger_transcations` WHERE RECEIPT_ID = $ET_RECEIPT_ID";    
		$queryFinTrans = $this->db->query($sqlFinTrans);
		$financialLedgerDetails = $queryFinTrans->first_row();
		$replaceVoucherNo=$financialLedgerDetails->VOUCHER_NO;
		if ($queryFinTrans->num_rows() > 0) {
				$sql1="UPDATE financial_ledger_transcations SET FLT_DEPOSIT_PAYMENT_DATE='$dDate',`PAYMENT_STATUS` = 'Pending' where VOUCHER_NO = '$replaceVoucherNo'";
				$this->db->query($sql1);

				$sql1="UPDATE financial_ledger_transcations SET FGLH_ID='$bank' where VOUCHER_NO = '$replaceVoucherNo' AND RP_TYPE ='R2'" ;
				$this->db->query($sql1);
		} else {

			$sql = " SELECT ET_RECEIPT_ID,ET_RECEIPT_NAME,ET_RECEIPT_DATE, ET_RECEIPT_CATEGORY_ID, (ET_RECEIPT_PRICE + POSTAGE_PRICE) AS PRICE, ET_RECEIPT_PAYMENT_METHOD, FGLH_ID,ET_RECEIPT_ID,PAYMENT_STATUS,RECEIPT_ET_ID FROM `event_receipt` WHERE ET_RECEIPT_ID = $ET_RECEIPT_ID";    
			$query = $this->db->query($sql);
			$receiptDetails = $query->first_row();

			$CHEQUE_NO = $_POST['EUC_CHEQUE_NO'];
			$BANK_NAME = str_replace("'","\'",$_POST['EUC_BANK_NAME']);
			$BRANCH_NAME = str_replace("'","\'",$_POST['EUC_BRANCH_NAME']);
			$CHEQUE_DATE = $_POST['EUC_CHEQUE_DATE'];


			$dateTime = date('d-m-Y H:i:s A');
			$aidR = $bank;
			$RECEIPT_ID = $receiptDetails->ET_RECEIPT_ID;
			$catId = $receiptDetails->ET_RECEIPT_CATEGORY_ID;
			$amtsR = $receiptDetails->PRICE;
			$tDateR = $receiptDetails->ET_RECEIPT_DATE;
			$flt_user = $_SESSION['userId'];
			$RECEIPT_PAYMENT_METHOD = $receiptDetails->ET_RECEIPT_PAYMENT_METHOD;
			$PAYMENT_STATUS = $receiptDetails->PAYMENT_STATUS;
			$RECEIPT_NAME = $receiptDetails->ET_RECEIPT_NAME;
			$RECEIPT_ET_ID = $receiptDetails->RECEIPT_ET_ID;
