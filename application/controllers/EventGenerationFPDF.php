...
class EventGenerationFPDF extends CI_Controller
{
	...
	public function index() {
		...
		if($_POST['posSearch'] != "") {
			$where .= " AND ET_RECEIPT_NAME = '".$_POST['posSearch']."' OR ET_RECEIPT_PHONE = '".$_POST['posSearch']."'";
		}
		
		if($_POST['posDate'] != "") {
			$where .= " AND POSTAGE.DATE = '".$_POST['posDate']."'";
		}
		
		$query = "SELECT * FROM POSTAGE INNER JOIN EVENT_RECEIPT ON POSTAGE.RECEIPT_ID = EVENT_RECEIPT.ET_RECEIPT_ID INNER JOIN EVENT ON EVENT_RECEIPT.RECEIPT_ET_ID = EVENT.ET_ID WHERE ET_ACTIVE = 1 AND ".$where; 
		...
		foreach ($result->result() as $row) {
			...
			$code = 0001;
			$name = strtoupper($row->ET_RECEIPT_NAME);
			$exp_date = date('dMY', strtotime($row->ET_RECEIPT_DATE));
	...
	public function eventAddress() {
		...
		if(isset($_POST['allDates'])) {
			...
			if(@$radioOpt == "multiDate") {
				...
				for($i = 0; $i < count($allDates1); ++$i) {
					if($i == 0) {
						$queryString .= "ET_RECEIPT_DATE='".$allDates1[$i]."' and ET_RECEIPT_ACTIVE = 1 and ET_ACTIVE = 1 and ET_RECEIPT_ADDRESS !=''"; 
						//$queryString1 .= "ET_RECEIPT_DATE='".$allDates1[$i]."' and ET_RECEIPT_ACTIVE = 0"; 
					} else {
						$queryString .= " or ET_RECEIPT_DATE='".$allDates1[$i]."' and ET_RECEIPT_ACTIVE = 1 and ET_ACTIVE = 1 and ET_RECEIPT_ADDRESS !=''"; 
						//$queryString1 .= " or ET_RECEIPT_DATE='".$allDates1[$i]."' and ET_RECEIPT_ACTIVE = 0"; 
					}
				}
				...
				$condition = "ET_RECEIPT_DATE = '".$_SESSION['dateField']."' and ET_RECEIPT_ACTIVE = 1 and ET_ACTIVE = 1 and ET_RECEIPT_ADDRESS !=''";
				 
			}

		$date = date('d-m-Y');
		$where = "ET_RECEIPT_DATE = '".$date."'";
 		$query = "SELECT * FROM  EVENT_RECEIPT INNER JOIN event ON event.ET_ID = EVENT_RECEIPT.RECEIPT_ET_ID WHERE ".$condition; 
		$result = $this->db->query($query);
		$number_of_products = $result->num_rows();
		...
		for($rowF = 0; $rowF < $number_of_products; $rowF++) {
			...
			if($arrLabels[$rowF]['Add3'] != "" && $arrLabels[$rowF]['Add4'] != "") {
				@$column_code = wordwrap($arrLabels[$rowF]['Name'],37,"\n",true)."\n".wordwrap($arrLabels[$rowF]['Add1'],37,"\n",true)."\n".wordwrap($arrLabels[$rowF]['Add2'],37,"\n",true)."\n".wordwrap($arrLabels[$rowF]['Add3']."-".$arrLabels[$rowF]['Add4'],37,"\n",true);
			} else if ($arrLabels[$rowF]['Add3'] == "" && $arrLabels[$rowF]['Add4'] != "") {
				@$column_code = wordwrap($arrLabels[$rowF]['Name'],37,"\n",true)."\n".wordwrap($arrLabels[$rowF]['Add1'],37,"\n",true)."\n".wordwrap($arrLabels[$rowF]['Add2'],37,"\n",true)."\n".wordwrap($arrLabels[$rowF]['Add4'],37,"\n",true);
			} else if ($arrLabels[$rowF]['Add3'] != "" && $arrLabels[$rowF]['Add4'] == "") {
				@$column_code = wordwrap($arrLabels[$rowF]['Name'],37,"\n",true)."\n".wordwrap($arrLabels[$rowF]['Add1'],37,"\n",true)."\n".wordwrap($arrLabels[$rowF]['Add2'],37,"\n",true)."\n".wordwrap($arrLabels[$rowF]['Add3'],37,"\n",true);
			} else if ($arrLabels[$rowF]['Add3'] == "" && $arrLabels[$rowF]['Add4'] == "") {
				@$column_code = wordwrap($arrLabels[$rowF]['Name'],37,"\n",true)."\n".wordwrap($arrLabels[$rowF]['Add1'],37,"\n",true)."\n".wordwrap($arrLabels[$rowF]['Add2'],37,"\n",true);
			}
			...
			if($k == 1) {
				$this->pdf->SetXY(8,$x);
				$this->pdf->MultiCell(0, 3.1, $column_code, 0,'J');
			}
	...
	public function trustEventAddress() {
		...
		if(isset($_POST['allDates'])) {
			...
			if(@$radioOpt == "multiDate") {
				...
				for($i = 0; $i < count($allDates1); ++$i) {
					if($i == 0) {
						$queryString .= "TET_RECEIPT_DATE='".$allDates1[$i]."' and TET_RECEIPT_ACTIVE = 1 and TET_ACTIVE = 1 and TET_RECEIPT_ADDRESS !=''"; 
						//$queryString1 .= "ET_RECEIPT_DATE='".$allDates1[$i]."' and ET_RECEIPT_ACTIVE = 0"; 
					} else {
						$queryString .= " or TET_RECEIPT_DATE='".$allDates1[$i]."' and TET_RECEIPT_ACTIVE = 1 and TET_ACTIVE = 1 and TET_RECEIPT_ADDRESS !=''"; 
						//$queryString1 .= " or ET_RECEIPT_DATE='".$allDates1[$i]."' and ET_RECEIPT_ACTIVE = 0"; 
					}
				}
				$condition= $queryString;
				...
				$condition = "TET_RECEIPT_DATE = '".$_SESSION['dateField']."' and TET_RECEIPT_ACTIVE = 1 and TET_ACTIVE = 1 and TET_RECEIPT_ADDRESS !=''";
				 
			}

		$date = date('d-m-Y');
		$where = "ET_RECEIPT_DATE = '".$date."'";
 		$query = "SELECT * FROM  TRUST_EVENT_RECEIPT INNER JOIN trust_event ON trust_event.TET_ID = TRUST_EVENT_RECEIPT.RECEIPT_TET_ID WHERE ".$condition; 
		$result = $this->db->query($query);
		$number_of_products = $result->num_rows();
		...
		foreach ($result->result() as $row) {
			$code = 0001;
			$name = strtoupper($row->TET_RECEIPT_NAME);
			$exp_date = date('dMY', strtotime($row->TET_RECEIPT_DATE));
			$add1 = strtoupper($row->ADDRESS_LINE1);
			$add2 = strtoupper($row->ADDRESS_LINE2); 	
			$city = strtoupper($row->CITY); 	
			$add3 = strtoupper($row->COUNTRY); 			
