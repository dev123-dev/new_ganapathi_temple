<div class="container">
	...
	<div class="row form-group">
		<div class="col-lg-10 col-md-10 col-sm-10 col-xs-8">
			<span class="eventsFont2">Jeernodhara Inkind Receipt</span>
		</div>
		<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">
			<a style="width:24px; height:24px" class="pull-right img-responsive" href="<?=site_url()?>Jeernodhara/Jeernodhara_Inkind" title="Reset"><img title="Reset" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
	</div>
	...
	<div class="row">
	   <div class="col-md-6 text-left">
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="margin-bottom: 1.5em;">
				<div class="form-inline">
					<label><input style="margin-right: 6px;" id="postage" name="postage"  type="checkbox" value="1">Postage</label>
		...
		<div class="col-md-6 text-left">
			<div class="sevaDate">	
				<div <?php if(@$_SESSION['eventActiveCount'] == 0) echo "style='display:none;'"?> class="form-inline col-lg-12 col-md-12 col-sm-12 col-xs-12" style="margin-bottom: 1.5em;">
  ...
  <div class="modal-dialog modal-lg" role="document">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Jeernodhara Inkind Preview</h4>
		</div>
...
<script>
	...
	$('#submit2').on('click', function() {
		...
		let url = "<?=site_url()?>Receipt/receipt_jeernodhara_inkind_save"
		let tableItem = [];
		let tableQty = [];
		let tableItemId = [];
		let tablePrice = [];
	...
	/* function checkConfirmTime(date) {
		...
		$.ajax({
            type: "POST",
            url: (selection == 1) ? "<?php echo base_url();?>Receipt/check_eod_confirm_time ": "<?php echo base_url();?>Events/check_event_eod_confirm_time ",
            success: function (response) {
				if(response == 0){
					field_validation();
				} else {
	...
	function field_validation() {
		...
		//Validation For Table
		let url = "<?=site_url()?>Receipt/receipt_inkind_save"
		let tableItem = [];
		let tableQty = [];
		let tableItemId = [];
		let tablePrice = [];
	...
	function addRow() {
		...
		if(count != 0) {
			...
			tableContent[si-1] = '<td class="tableItemId" style="display:none">'+ itemid +'</td><td class="tableInkind">'+ items + '</td><td class="tableQty">'+ qty + '</td><td class="tableQty">'+ estimatedprice + '</td><td class="tableQty">'+ paymentNotes + '</td>';
			
			$('#inkind-list').append('<tr><td>'+si+'</td><td class="tableItemId" style="display:none">'+ itemid +'</td><td class="tableInkind">'+ items + '</td><td class="tableQty">'+ qty + '</td><td class="tablePrice">'+ estimatedprice + '</td><td class="tableNotes">'+ paymentNotes + '</td><td class="tableItemId"><a style="cursor:pointer;" onClick="updateTable('+ (si-1) +');"><img style="width:24px; height:24px;" title="delete" src="<?php echo base_url();?>images/delete1.svg"></td></tr>');
		}
	}
	...
	function updateTable(si) {
		...
		for(var i = 0, j = 1, s = 0; i < tableContent.length; i++) {
			...
			console.log("tableContent4" + tableContent4);
			console.log("tableContent5" + tableContent5);
			if(i != si) {
					tableContent2[s] = '<tr><td>'+ j +'</td>' + tableContent[i] + '<td ><a style="cursor:pointer;" onClick="updateTable('+ s +');"><img style="width:24px; height:24px;" title="delete" src="<?php echo base_url();?>images/delete1.svg"></td></tr>';
