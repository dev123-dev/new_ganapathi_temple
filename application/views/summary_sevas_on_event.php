...
<div style="clear:both;" class="container">
	...
	<div class="row form-group">							
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">	
			<span class="eventsFont2">Event Seva Summary</span>
			<a class="pull-right" style="margin-left:5px;" href="<?=site_url()?>Report/summary_sevas_on_event" title="Refresh"><img style="width:24px; height:24px;" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
	</div>	
...
<div class="container">
				...
				<table class="table table-bordered table-hover">
					...
					<tbody>
						<?php $Count = 0; foreach($report_details as $result) { 
							if($result['ET_RECEIPT_ACTIVE'] != '0') {
							$Count += $result['QTY'];						?>
							<tr class="row1">
								<td><a style="color:#800000;" onClick = "get_display_deity_details('<?php echo $result['ET_SO_SEVA_ID']; ?>','<?php echo $result['ET_SO_SEVA_NAME']; ?>')"><?php echo $result['ET_SO_SEVA_NAME']; ?></a></td>
	...
	<div class="row form-group">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<label class="pull-right" for="sevaAmount" style="font-size:18px;margin-left:15px;margin-top: -1em;">Total Sevas: <span id="totalSeva"><?php echo @$Count; ?></span></label>  
		</div>
	</div>
</div>
...
<script>
	...
	$('#EveryRadio').on('click', function() {
		...
		document.getElementById('radioOptField').value = "multiDate";
		url = "<?php echo site_url(); ?>Report/summary_sevas_on_event";
	...
	function get_display_deity_details(Id,Name) {
		...
		url = "<?php echo site_url(); ?>Report/event_sevas_summary_details";
	...
	//ON CHANGE OF DATEFIELD
	 function get_datefield_change(date) {
		...
		url = "<?php echo site_url(); ?>Report/get_filter_change_event";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	} 
	...
	function GetSendField() {
		...
		url = "<?php echo site_url(); ?>Report/event_sevas_summary_report_excel";
	...
	function generatePDF() {
		...
		url = "<?php echo site_url(); ?>generatePDF/create_eventSevaSummaryReceiptpdf";
		$("#report").attr("action",url)
		$("#report").submit();
	}
	...
	//CREATE PRINT
	function print() {
		let url = "<?php echo site_url(); ?>generatePDF/create_eventSevaSummaryReceiptSession";
		...
		$.post(url,{'dateField':$('#todayDate').val(),'radioOpt':radioOpt,'fromDates':fromDates,'toDates':toDates}, function(data) {
			let url2 = "<?php echo site_url(); ?>generatePDF/create_eventSevaSummaryReceiptPrint";
			if(data == 1) {
				downloadClicked = 0;
				var win = window.open(
				  url2,
