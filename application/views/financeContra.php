...
<div class="container-fluid container">
    ...
    <div class="row form-group">
        ...
        <div class="col-lg-3 col-md-10 col-sm-10 col-xs-8">
            <form id="frmCommitteeChange" action="<?=site_url()?>finance/Contra" method="post">
        ...
        <div class="col-lg-2 col-md-2 col-sm-2 col-xs-4" style = "padding-top:10px;">
            <a style="text-decoration:none;cursor:pointer;pull-right;" href="<?=site_url()?>finance/Contra" title="Refresh"><img style="width:24px; height:24px" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
        </div>
        
    </div>
    <form id="frmContra" action="<?=site_url()?>finance/addContraTrans" method="post" enctype="multipart/form-data"
                            ...
                            <tbody>
                                ...
                                $selectedLedgerId= ""; 
                                 foreach($cashReceipts as $result) { ?>
                                    <tr class="parent" id="row<?php echo $result->FGLH_ID; ?>" name="<?php echo $result->BankLedgerId; ?>" title="Click to expand/collapse" style="cursor: pointer;">
                                        <td><center><input type="checkbox" class="rowcheckbox" onclick="mainCheckboxClick('<?php echo $result->FGLH_ID; ?>')" name="<?php echo $result->BankLedgerId; ?>" id="checkboxrow<?php echo $result->FGLH_ID; ?>" /></center></td>
                                        <td><input type="text" class="ledgerGroupName" id="ledgerGroupName_<?php echo $result->FGLH_ID; ?>" value="<?php echo $result->FGLH_NAME; ?>" style="width:100% ;background: transparent; border:none;cursor: pointer" readonly/></td>
                                        <td class="text-right "><?php echo $result->AMOUNT; ?></td>
                                        <td></td>
                                        <td><input type="text" class="text-right ledgerGroupTotAmt" name="<?php echo $result->FGLH_ID; ?>" id="selLedgerTotAmt_<?php echo $result->FGLH_ID; ?>" style="width:100% ;background: transparent; border:none;" readonly/> </td>
                                    </tr> 
                                        <?php
                                        $i=1;
                                         foreach($indCashReceipts as $row) { 
                                            if($result->FGLH_ID==$row->FGLH_ID) {
                                            ?>
                                            <tr class="child-row<?php echo $result->FGLH_ID; ?>" style="display: none;">
                                                <td></td>    
                                                <td class="text-right"><?php echo $row->FLT_DATE; ?></td>
                                                <td class="text-right"><?php echo $row->AMOUNT; ?></td>
                                                <td><center><input type="checkbox" name="<?php echo $row->BankLedgerId; ?>" id="<?php echo $row->FGLH_ID;?>_<?php echo $i ?>" onchange="GetSelectLedgerOption('<?php echo $row->FGLH_ID; ?>','<?php echo $row->AMOUNT; ?>','<?php echo $i ?>','<?php echo $row->FLT_ID; ?>')" class="sel <?php echo $row->FGLH_ID; ?>" /></center></td>
                                                <td></td>
                                            </tr>
                                        <?php  $i++;
                                         } 
...
<script type="text/javascript">
    ...
    function AccountChange(){
        ...
        if(acc1[0] == 21){
            ...
            /*$("#acidC option").each(function () {
                
                if (this.value.split('|')[0] == '27') {
                    $("#acidC option:contains(this)").attr("disabled",true);
                    //this.disabled = true;
                }
            });*/
        }
        ...
        if (e1[0] == "success"){
            ...
            let url2 = "<?php echo site_url(); ?>generatePDF/create_ContraPrint";
                if(data == 1) {
                    ...
                    location.href = "<?=site_url();?>finance/Contra";
                }
            })
        }
        else
