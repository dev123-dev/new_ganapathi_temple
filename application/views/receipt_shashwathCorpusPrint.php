...
<div id="printScreen" style="display:none">
	<page style="margin-top:25px;width:715px;margin-left:7%;">
		<form>
			<center class="eventsFont2" style="margin-top:74px;font-size:14px;font-family:switzerland;"><strong><?=$templename[0]["TEMPLE_NAME"]?></strong></center>
			<center class="eventsFont2" style="display:none;margin-top:65px;font-size:11px;padding-bottom:4px;" id="original"><strong>Shashwath Receipt</strong></center>
			<center class="eventsFont2" style="display:none;margin-top:65px;font-size:11px;padding-bottom:4px;" id="duplicate"><strong><?php if( $deityCounter[0]["PRINT_STATUS"]== 1) echo "Duplicate Shashwath Receipt "?></strong></center>
				...
				<table >
					...
					<td width="74%"><span style="font-size:11px;letter-spacing:1px;margin-left:20px;float:right;" class="eventsFont2"><b>Receipt Date&nbsp;:</b> <?=$deityCounter[0]["RECEIPT_DATE"];?></span></td>
					</tr>
					<tr>
					<td width="74%">
				...
				<?php if($deityCounter[0]['SM_RASHI'] != "") { ?>
						<span style="font-size:12px;letter-spacing:1px;"><strong>Rashi&nbsp;:</strong><?=$deityCounter[0]['SM_RASHI'] ?></span>
					<?php }else {?>
						<span style="font-size:12px;letter-spacing:1px;"><strong>Rashi&nbsp;:</strong></span>
						<?php }?>
			...
			<center>
				<table border="1" style="margin-top:15px; margin:15px">
					<tr>
			...
			<div style="margin-top:5px;"></div>
			<span style="font-size:11px;letter-spacing:1px;"><strong>Total Amount: Rs. <?=$deityCounter[0]['RECEIPT_PRICE'] ?>/-<?=AmountInWords($deityCounter[0]['RECEIPT_PRICE']);?></strong></span><br/>
			...
			<?php } ?>
			<span style="font-size:12px;letter-spacing:1px;margin-top:30px;margin-left:420px;"><strong>Issued By&nbsp;: </strong><?=$deityCounter[0]['RECEIPT_ISSUED_BY'] ?></span><br/>
...
<form id="backToPage" method="post">
	...
	<input type="hidden" name="soVal" value = "<?=$deityCounter[0]['SO_ID'] ?>" id="soVal"/>
</form>
...
<script>
    function backToPage(){
		if('<?php echo $_SESSION['CorpusCallFrom']; ?>' == "LossDetail") {
			$('#backToPage').attr('action', "<?=site_url() ?>Shashwath/lossDetail").submit();
		} else if('<?php echo $_SESSION['CorpusCallFrom']; ?>' == "shashwathMember") {
			window.location.href = "<?=site_url() ?>Shashwath/shashwath_member";
		} else {
	...
	console.log("heyyyyy",<?php echo $deityCounter[0]['PRINT_STATUS']?>)
