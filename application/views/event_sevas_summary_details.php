...
<script>
	...
	function get_datefield_change(date) {
		...
		url = "<?php echo site_url(); ?>Report/get_filter_change_event_sevas_details";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	}
	...
	//CREATE EXCEL
 	function GetSendField() {
		...
		url = "<?php echo site_url(); ?>Report/event_sevas_summary_report_details_excel";
	...
	function generatePDF() {
		...
		url = "<?php echo site_url(); ?>generatePDF/create_eventSevaSummaryDetailspdf";
		$("#report").attr("action",url)
		$("#report").submit();
	}
	...
	//CREATE PRINT
	function print() {
		let url = "<?php echo site_url(); ?>generatePDF/create_eventSevasSummaryDetailsSession";
		...
		$.post(url,{'dateField':$('#todayDate').val(),'radioOpt':radioOpt,'fromDates':fromDates,'toDates':toDates,'sevaId':sevaId,'sevaName':sevaName}, function(data) {
			let url2 = "<?php echo site_url(); ?>generatePDF/create_eventSevasSummaryDetailsPrint";
			if(data == 1) {
				downloadClicked = 0;
				var win = window.open(
				  url2,
