								...
								<table class="table table-bordered table-hover">
									<thead>
										<tr>
											...
											<th style="width:15%;"><center><strong>Receipt Category</center></strong></th>
									...
									<tbody>
										...
										foreach($addbank as $result) {?>												
											<tr class="row1">
												<?php
												echo "<td class='rcp_" . $i . "'><center>". $result->RECEIPT_ID."</center></td>";
												?>
...
<script>
	function updateBank(i,rcpid){
		...
		if (count != 0) {
			...
			$('#id').val(rcpid);
			$('#bank1').val(bank1);
			$('#submitForm').attr('action','<?=site_url()?>finance/updateBankDetails');
			$('#submitForm').submit();
		}
	}
