...
<div class="container">
	...
	<div class="row form-group">
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 marginTop">
			<span class="samFont1"><?=$event['ET_NAME']; ?></span>
		</div>
		<form action="<?=site_url();?>Events/searchSeva" id="dateChange" enctype="multipart/form-data" method="post" accept-charset="utf-8" onsubmit="return field_validation();">
			<div class="col-lg-2 col-md-3 col-sm-4 col-xs-6">
				<div class="input-group input-group-sm">
					 <input type="hidden" name="date" value="<?=@$date; ?>" id="date" value="">
					<input type="hidden" name="load" id="load" value="">
					<input autocomplete="" id="todayDate" type="text" value="<?=@$date; ?>" class="form-control todayDate2"  onchange="GetDataOnDate(this.value,'<?php echo site_url()?>Events/searchSeva/')" placeholder="dd-mm-yyyy" readonly ="readonly"/>
		...
		<div class="col-offset-lg-6 col-lg-1 col-md-2 col-sm-4 col-xs-12 pull-right text-right">
			<?php if(isset($_SESSION['Add'])) { ?>
				<a style="margin-left: 9px;pull-right;" href="<?=site_url()?>Events/event_receipt" title="Add Event Seva"><img style="width:24px; height:24px" src="<?=site_url();?>images/add_icon.svg"/></a>
			<?php } ?>
			<a style="text-decoration:none;cursor:pointer;pull-right;" href="<?=site_url()?>Events" title="Refresh"><img style="width:24px; height:24px" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
		</form>
	</div>
</div>
					...
					<thead>
					 ...
					 <tbody>
					   <?php foreach($eventSeva as $result) {
						   $receiptNo = $result['ET_RECEIPT_NO'];
							echo "<tr>";
							echo "<td><a style='cursor:pointer;color:#800000;text-decoration:none;' onClick='printReceipt(`$receiptNo`,1)'>".$result['ET_RECEIPT_NO']."</a></td>";
							if($result['ET_RECEIPT_PHONE'] != "") 
								echo "<td>".$result['ET_RECEIPT_NAME']. " (".$result['ET_RECEIPT_PHONE'].")"."</td>";
							else
								echo "<td>".$result['ET_RECEIPT_NAME']."</td>";
							if($result['ET_RECEIPT_NAKSHATRA'] != "") {
								echo "<td>".$result['ET_RECEIPT_RASHI']. " (".$result['ET_RECEIPT_NAKSHATRA'].")"."</td>";
							}else
								echo "<td>".$result['ET_RECEIPT_RASHI']."</td>";
		...
		<form id="printReceipt" method="post" action="<?=site_url()?>Events/editReceipt/">
		...
		<script>
			...
			function printReceipt(receiptNo, num) {
				if(num == 1) {
					...
					$('#printReceipt').attr('action',"<?=site_url()?>Events/printSevaReceipt/");
					$('#printReceipt').submit();
				}else {
					$('#receiptNo').val(receiptNo);
					$('#printReceipt').attr('action',"<?=site_url()?>Events/editReceipt/");
					$('#printReceipt').submit();
				}
			}
