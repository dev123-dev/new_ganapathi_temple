	...
	<div class="row form-group">
		...
		<form id="approveId" enctype="multipart/form-data" method="post" accept-charset="utf-8">
			...
			<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">			
				<a class="pull-right img-responsive" onClick="event.preventDefault(); goBackEod()" title="back"><img style="width:24px; height:24px;margin-right:10px;cursor:pointer" title="Back" src="<?=site_url();?>images/back_icon.svg"/>
				</a>
				
				<a class="pull-right" onclick="GetRefresh('<?=site_url()?>EOD/deityEod_usercollection')" title="Refresh"><img style="width:24px; height:24px;margin-right:20px;cursor:pointer" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
				<?php if(isset($_SESSION['Authorise'])) { ?>
					<a class="pull-right" onclick="approveSubmit('<?php echo $UserDate ?>','<?php echo @$UserName ?>','<?php echo @$UserId ?>')" title="Authorise"><img style="width:24px; height:24px;margin-right:8px;cursor:pointer" title="Authorise " src="<?=site_url();?>images/check_icon.svg"/></a>
				<?php } ?>
			</div>
		</form>
	</div>
	<div class="row form-group">
		<form id="userFilters" enctype="multipart/form-data" method="post" accept-charset="utf-8">
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
			  <select id="modeOfPayment" name="modeOfPayment" class="form-control" onchange="GetDataOnFilter(this.value,'<?php echo site_url()?>EOD/get_data_on_filter/1','<?php echo $UserDate ?>','<?php echo $UserName ?>','<?php echo $UserId ?>')">
		...
		<div class="control-inline col-md-9 col-lg-9 col-sm-12 col-xs-12 text-right" style="font-size:15px;margin-top:.2em;">
...
<script>
	...
	function goBackEod(){
		clearSessionValues();
		document.getElementById('backEod').submit();
	}
	...
	$(document).ready(function() {
   		...
   		let arrTotDBItems = <?php echo $all_receipt_report; ?>;
	    ...
	    if(sessionStorage.getItem("SelectedReceiptID") !== null && sessionStorage.getItem("SelectedReceiptID") != "") {
	   	...
	   	arrItems = <?php echo $receipt_report; ?>;
	   	$('#tblEod').html("");
	   	$('#eodContent').html('<div class="table-responsive" style="overflow-x:hidden"><table class="table "><thead><tr><th style="border:1px solid #7d6363"><center></center></th><th style="border:1px solid #7d6363">Receipt No.</th><th style="border:1px solid #7d6363">Receipt Type</th><th style="border:1px solid #7d6363"><center>Receipt Name</center></th><th style="border:1px solid #7d6363"><center>Payment Mode</center></th><th style="border:1px solid #7d6363"><center>Amount</center></th><th style="border:1px solid #7d6363"><center>Postage</center></th><th style="border:1px solid #7d6363"><center>Grand Total</center></th><th style="border:1px solid #7d6363"><center>Payment Status</center></th></tr></thead><tbody id="tblEod"></tbody></table></div>')
	    for(var i = 0; i < arrItems.length; i++) {
    		$('#tblEod').append('<tr>');
    		$('#tblEod').append('<input type="hidden" name="pageVal" id="pageVal"/>');

    		if(sessionStorage.getItem("SelectAll") !== null && sessionStorage.getItem("SelectAll") == "true") {
    			$("#all_users").prop('checked', true);
    			$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px"><center><input id="checkerId_'+i+'" name="checkerId_'+i+'" type="checkbox" onchange="GetUnselect('+arrItems[i]["RECEIPT_ID"]+',this.checked)" checked class="sel" /></center></td><td class="recId" id="recid_'+i+'" style="display:none;">'+arrItems[i]["RECEIPT_ID"]+'</td>');
    		} else {
	    		if(sessionStorage.getItem("SelectedReceiptID") !== null && sessionStorage.getItem("SelectedReceiptID") != "") {
	    			if(arrSelectedReceiptID.indexOf(parseInt(arrItems[i]["RECEIPT_ID"])) > -1) {
						...
						$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px"><center><input id="checkerId_'+i+'" name="checkerId_'+i+'" type="checkbox" onchange="GetUnselect('+arrItems[i]["RECEIPT_ID"]+',this.checked)" class="sel" /></center></td><td class="recId" id="recid_'+i+'" style="display:none;">'+arrItems[i]["RECEIPT_ID"]+'</td>');
					}
				}else{
					$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px"><center><input id="checkerId_'+i+'" name="checkerId_'+i+'" type="checkbox" onchange="GetUnselect('+arrItems[i]["RECEIPT_ID"]+',this.checked)" class="sel" /></center></td><td class="recId" id="recid_'+i+'" style="display:none;">'+arrItems[i]["RECEIPT_ID"]+'</td>');
				}
			...
			$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["RECEIPT_NO"] + '</td><td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["RECEIPT_CATEGORY_TYPE"]+'</td><td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["RECEIPT_NAME"]+'</td>');

			if(arrItems[i]["RECEIPT_PAYMENT_METHOD"] == "Cheque") {
				$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px"><a class="log mymodelcancel" style="color:#800000;" onclick="show_cheque(1,\''+arrItems[i]["CHEQUE_NO"]+'\',\''+arrItems[i]["CHEQUE_DATE"]+'\',\''+arrItems[i]["BANK_NAME"]+'\',\''+arrItems[i]["BRANCH_NAME"]+'\',\''+arrItems[i]["TRANSACTION_ID"]+'\')">'+arrItems[i]["RECEIPT_PAYMENT_METHOD"]+'</a></td>');
    		} else if (arrItems[i]["RECEIPT_PAYMENT_METHOD"] == "Credit / Debit Card") {
    			$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px"><a class="log mymodelcancel" style="color:#800000;" onclick="show_cheque(2,\''+arrItems[i]["CHEQUE_NO"]+'\',\''+arrItems[i]["CHEQUE_DATE"]+'\',\''+arrItems[i]["BANK_NAME"]+'\',\''+arrItems[i]["BRANCH_NAME"]+'\',\''+arrItems[i]["TRANSACTION_ID"]+'\')">'+arrItems[i]["RECEIPT_PAYMENT_METHOD"]+'</a></td>');
    		} else {
    			$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["RECEIPT_PAYMENT_METHOD"]+'</td>');
    		}
    		if(arrItems[i]["RECEIPT_CATEGORY_TYPE"] != 'Inkind') { 
    			$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["RECEIPT_PRICE"]+'</td>');
    		} else {
    			$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px">-</td>');
    		}
    		if(arrItems[i]["RECEIPT_CATEGORY_TYPE"] == 'Seva') {
    			$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["POSTAGE_PRICE"]+'</td>');
    		} else {
    			$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px">-</td>');
    		}
    		if(arrItems[i]["RECEIPT_CATEGORY_TYPE"] != 'Inkind') {
    			$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px">'+(parseInt(arrItems[i]["RECEIPT_PRICE"]) + parseInt(arrItems[i]["POSTAGE_PRICE"]))+'</td>');
    		} else { 
    			$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px">-</td> ');
    		}
    		$('#tblEod').append('<td style="border:1px solid #7d6363;padding:10px">'+ arrItems[i]["PAYMENT_STATUS"] +'</td>');
			$('#tblEod').append('</tr><br/>');
	    }
	    GetCheckedItems();
	});	
	...
	function selectCheckOptions(eleId, checker) {
		if(eleId == "all_users") {
			...
			$('#tblEod').find("td.recId").each(function(index) {
				if(checker)
