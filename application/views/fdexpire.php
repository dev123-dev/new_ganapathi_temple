	...
	<form action="" id="dateChange" enctype="multipart/form-data" method="post" accept-charset="utf-8">
		...
		<div class="row form-group" style="margin-top:-0.5em">	
			...
			<div class="col-lg-8 col-md-6 col-sm-6 col-xs-12 pull-right text-right" style="padding-right:0px;">
				<a style="text-decoration:none;cursor:pointer;pull-right;" href="<?=site_url()?>finance/Fddetails" title="Refresh"><img style="width:24px; height:24px;margin-top:-0.3em;" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
			</div>
		</div>	
	</div>
</div>
					...
					<tbody>
						<?php if(isset($fdExpCurrentMonth)){ $count =0;?>
							<?php foreach($fdExpCurrentMonth as $result) {
								$d1=$result->FD_MATURITY_START_DATE;
								$d2=$result->FD_MATURITY_END_DATE;
								...
								$FGLH_NAME =  $result->FGLH_NAME;
								$FD_BANK_ID = $result->FD_BANK_ID;
								$FD_BANK_NAME = $result->FD_BANK_NAME;
								$FD_NUMBER = $result->FD_NUMBER;
								?> 
								<tr>
								
								<td width="20%"><a onclick="openFdDetailsWindow('<?=$result->FGLH_ID?>')" style="text-decoration:underline;color: #681212" >
								<?php echo $result->FGLH_NAME;?></a></td>
									<td width="10%"><?php echo $result->FD_BANK_NAME;?></td>
									<td width="10%"><?php echo $result->FD_MATURITY_START_DATE;?></td>
									
									<td width="10%"><?php echo $result->FD_MATURITY_END_DATE;?></td>
									<td width="10%"><?php echo $result->FD_INTEREST_RATE;?></td>
									<td width="10%"><?php echo $diff;?></td>
									<td width="10%"><?php echo $result->FLT_DR;?></td>
									<td width="7%">
								
									<!-- <a style="margin-left: 0px;" title="Add Interest"> -->
									  <!-- <img class="" style=" height:24px" onclick="addInterestModal('<?=$result->FGLH_ID ?>','<?=$FGLH_NAME ;?>','<?=$FD_BANK_ID ?>','<?=$FD_BANK_NAME ?>','<?=$FD_NUMBER ?>')" src="<?=site_url();?>images/add_icon.svg"/> -->
								   <!-- </a> -->
								  
								   
								   <!-- <a style="margin-left: 0px;" title="Edit Interest"> -->
									  <img class=" " style=" height:24px" onClick="editLedger('<?=$result->FGLH_ID; ?>', 
									                                                          '<?php echo str_replace("'","\'",$result->FGLH_NAME);?>',
																							  '<?=$result->FGLH_PARENT_ID; ?>',
																							  '<?=$result->IS_JOURNAL_STATUS; ?>',
																							  '<?=$result->IS_TERMINAL; ?>',
																							  '<?=$result->IS_FD_STATUS; ?>',
																							  '<?=$result->FD_MATURITY_START_DATE; ?>',
																							  '<?=$result->FD_MATURITY_END_DATE; ?>',
																							  '<?=$result->FD_INTEREST_RATE; ?>'
																							  )"
								   ...
								   <!-- </a> -->
									...
									<!-- <td width="20%">
										<?php echo ($result->FLT_DR)*(($diff)/12)*($result->FD_INTEREST_RATE)/100;?>
									</td> -->
								</tr>
							<?php }
						} ?>
					...
					<tbody>
						<?php if(isset($fdAllExp)){ $count =0;?>
							<?php foreach($fdAllExp as $result) { 
								...
								?>
								<tr>
									<td width="10%"><?php echo $result->FGLH_NAME;?> </td>
									<td width="20%"><?php echo $result->FD_MATURITY_START_DATE;?></td>
									
									<td width="20%"><?php echo $result->FD_MATURITY_END_DATE;?></td>
									<td width="20%"><?php echo $result->FD_INTEREST_RATE;?></td>
									<td width="20%"><?php echo $diff;?></td>
									<td width="20%"><?php echo $result->FLT_DR;?></td>
									<td width="20%"><?php echo ($result->FLT_DR)*(($diff)/12)*($result->FD_INTEREST_RATE)/100;?></td>	
								</tr>
							<?php }
						} ?>
					</tbody>
...
<script type="text/javascript">	
	...
	function saveInterest(){
		...
		$.post("<?=site_url()?>finance/addInterest",{'bank':FGLH_NAME,'fd_bank_name':fd_bank_name,'fd_number':fd_number,'fd_bank_id':fd_bank_id,'fd_interest_date':fd_interest_date,'fd_interest':fd_interest,'fglh_id':fglh_id}, 
		 function(e) {
...
//          url: "<?php echo base_url();?>finance/getInterestDetails",
