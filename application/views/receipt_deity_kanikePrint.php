...
<div id="A5print" style="display:none;">
	<section class="sheet padding-10mm">
		<!-- <img src="http://www.visitnorwich.co.uk/assets/Uploads/Events-images/Theatre-generic.jpg" class="testBack"> -->
		<strong>
			Test Name.<br/>
			Test ORG.<br/>
...
<!--for printing --><div id="printScreen" style="display:none">
	<page style="margin-top:25px;margin-left:75%;width:115%;margin-right:75%;">
		<form><br/>
			...
			<center style="margin-top:75px;"><span class="eventsFont2" style="display:none;font-size:11px;padding-bottom:4px;" id="duplicate"><strong><?="Duplicate";?>&nbsp;</strong></span><span class="eventsFont2" style="font-size:11px;padding-bottom:4px;"><strong>Kanike Receipt For <?=$deityCounter[0]["KANIKE_NAME"]?></strong></span></center>
			...
			<span style="font-size:11px;"><strong>Kanike for&nbsp;: <?=$deityCounter[0]["KANIKE_NAME"]; ?></span></strong><br/>
			...
			<?php if($deityCounter[0]['POSTAGE_PRICE'] != 0) { ?>
				<span class="eventsFont2"><strong>Postage: Rs. <?=$deityCounter[0]['POSTAGE_PRICE']; ?> <?=AmountInWords($deityCounter[0]['POSTAGE_PRICE']);?></strong></span><br/>
			<?php } ?>
...
<!--<page>
		<form>
			<center><span class="eventsFont2 samFont1"><?=$deityCounter[0]["RECEIPT_NAME"]; ?></span></center>
			...
			<?php if($deityCounter[0]['RECEIPT_PHONE'] != "") { ?>
				<?php //$phone = ", " .$deityCounter[0]['RECEIPT_PHONE']; ?>
			<?php } ?>
			<?php if($deityCounter[0]['RECEIPT_EMAIL'] != "") { ?>
				<?php //$email = ", ".$deityCounter[0]['RECEIPT_EMAIL']; ?>
			<?php } ?>
			<?php if($deityCounter[0]['RECEIPT_ADDRESS'] != "") { ?>
					<?php //$address = .", ".$deityCounter[0]['RECEIPT_ADDRESS']; ?>
				<?php } ?>
			<center><span class="eventsFont2">Donation/Kanike Receipt</span></center><br/>
			<span class="eventsFont2">Receipt Date: <?=$deityCounter[0]["RECEIPT_DATE"];?></span><br/>
			<span class="eventsFont2">Receipt Number: <?=$deityCounter[0]['RECEIPT_NO'] ?></span><br/>
			<span style="font-size:18px;"><strong>Received with thanks from: </strong><?=$deityCounter[0]['RECEIPT_NAME']. "" . @$phone. "".@$email."". @$address; ?></span><br/>
			<span class="eventsFont2">a sum of Rupees: <?=$deityCounter[0]['RECEIPT_PRICE'] ?></span><br/>
			<span style="font-size:18px"><strong>By cash/D.D/cheque: </strong><?=$deityCounter[0]['RECEIPT_PAYMENT_METHOD']; ?></span><br/>
...
<div class="container">
	<form class="form-group">
		<div class="row form-group">
			<div class="col-lg-6 eventsFont2" style="font-size:25px;">Deity Kanike Print</div>
			<div class="col-lg-6 " style="margin-top:0.5em;margin-bottom:3em;padding-right:15px;">
				<div class="form-group">
					...
					<?php } else { ?>
						<a class="pull-right" style="border:none; outline:0;" href="<?=site_url() ?>Receipt/receipt_deity_kanike" title="Back"><img style="border:none; outline: 0;" src="<?php echo base_url(); ?>images/back_icon.svg"></a>
					<?php } ?>
						<!-- <?php if(isset($_SESSION['bookingKanike'])) { ?>
							<a class="pull-right" style="border:none; outline:0;" href="<?=site_url() ?>Receipt/editReceipt/" title="Back"><img style="border:none; outline: 0;" src="<?php echo base_url(); ?>images/back_icon.svg"></a>
							<?php } else { ?> -->
							<!-- <?php } ?>	 -->
						</div>
					</div>
				...
				<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12" style="padding-left:0px">
					...
					<div class="form-group">
						<span style="font-size:18px;"><strong>Kanike for: </strong><?=$deityCounter[0]["KANIKE_NAME"]; ?></span>
					</div>
		...
		<form id="submitForm" action="<?php echo site_url(); ?>Receipt/save_cancel_note_kanike/" class="form-group" role="form" enctype="multipart/form-data" method="post">
	...
	var print = function() {
		...
		let refPrice = Number("<?=@$DEITY_KANIKE_SPECIAL_RECEIPT_PRICE->PRICE; ?>");
		
		if(paidPrice >= refPrice)
			confirmMultiPrintKanikeReceipt();
		else {
