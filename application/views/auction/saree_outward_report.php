...
<div class="container">
	...
	<div class="row form-group">
		<div class="col-lg-10 col-md-10 col-sm-10 col-xs-10">
			<span class="eventsFont2">Saree Outward Report: <span class="samFont"><?php echo $events[0]->ET_NAME; ?></span></span>
		</div>
		<div class="col-lg-2 col-md-2 col-sm-2 col-xs-2" style="margin-top: 1em;">
			<a style="width:24px; height:24px" class="pull-right img-responsive" href="<?=site_url()?>Report/saree_outward_report" title="Refresh"><img title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
	</div>
...
<script>
	...
	$('#submit').on('click', function() {
		...
		if(opt == "Check Availability") {
			url = "<?=site_url(); ?>Auction/checkSareeDate"
			...
			if(date.getTime() == date12.getTime()) {
				...
				url = "<?=site_url(); ?>Auction/updateSareeDate"
				$.post(url,{'itemId':itemId,'sareeType':sareeType,'event_name':eventName,'sevadate':sevadate}, function(e) {
					alert("Information",e)
					$('.modal').modal("hide");	
				});
	...
	//FOR DATEFIELD
	var currentTime = new Date()
	var fromDate = "<?=$events[0]->ET_FROM_DATE_TIME; ?>";
	var toDate = "<?=$events[0]->ET_TO_DATE_TIME; ?>"; 
	fromDate = fromDate.split("-");
	toDate = toDate.split("-");
	var minDate = new Date(Number(fromDate[2]), (Number(fromDate[1])-1), +(Number(fromDate[0]))); 
	var maxDate =  new Date(Number(toDate[2]), (Number(toDate[1])-1), + Number(toDate[0]));
	...
	function print() {
		...
		let url = "<?php echo site_url(); ?>generatePDF/create_auctionItemSession";
		$.post(url,{'date':$('#todayDate').val()}, function(data) {
			let url2 = "<?php echo site_url(); ?>generatePDF/create_auctionItemPrint";
			if(data == 1) {
				downloadClicked = 0;
				var win = window.open(
				  url2,
	...
	function generatePDF() {
		...
		url = "<?php echo site_url(); ?>generatePDF/create_auctionItemPDF";
	...
	function GetSendField() {
		...
		document.getElementById('tdate').value = $('#todayDate').val();
		url = "<?php echo site_url(); ?>Report/event_auction_report_excel";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	}
	...
	function get_datefield_change(date) {
		...
		url = "<?php echo site_url(); ?>Report/get_change_datefield";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	}
</script>
