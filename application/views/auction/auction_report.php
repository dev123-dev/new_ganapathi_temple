<div class="container">
	...
	<div class="row form-group">
		<div class="col-lg-10 col-md-10 col-sm-10 col-xs-10">
			<span class="eventsFont2">Auction Report: <span class="samFont"><?php echo $events[0]->ET_NAME; ?></span></span>
		</div>
...
<script>
	function print() {
		let url = "<?php echo site_url(); ?>generatePDF/create_auctionSession";
		$.post(url,{'payMode':$('#modeOfPayment').val()}, function(data) {
			let url2 = "<?php echo site_url(); ?>generatePDF/create_auctionReportPrint";
			if(data == 1) {
					downloadClicked = 0;
					var win = window.open(
					  url2,
	...
	function generatePDF() {		
		...
		url = "<?php echo site_url(); ?>generatePDF/create_auctionreportpdf";
	...
	function GetSendField(paymentMethod) {		
		document.getElementById('payMode').value = $('#modeOfPayment').val();
		url = "<?php echo site_url(); ?>Report/auction_report_excel";
		$("#report").attr("action",url)
		$("#report").submit();
	}
	...
	function get_payment_mode_change(paymentMode) {
		...
		url = "<?php echo site_url(); ?>Report/auction_report_payment_mode";
