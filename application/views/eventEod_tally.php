...
<div class="container">
	...
	<div class="row form-group">
		<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
			<span class="eventsFont2">Event E.O.D. Tally</span>
		</div>
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="table-responsive">
				<table class="table table-bordered table-hover">
					<thead>
					  <tr>
						<th>Event Receipt Date</th>
						<th>Event Receipt Name</th>
						<th>Event Receipt Category</th>
						<th>Cheque No</th>
						<th>Bank</th>
						<th>Cheque Amount</th>
					...
					<tbody>
						<?php foreach($eod_tally as $result) { ?>
							<tr class="row1">
								...
								<td><?php echo date('d-m-Y',strtotime($result->EUC_EOD_DATE)); ?></td>
								<td><?php if($result->ET_RECEIPT_NAME != NULL) echo $result->ET_RECEIPT_NAME; else echo "-"; ?></td>
								<td><?php if($result->ET_RECEIPT_CATEGORY_TYPE != NULL) echo $result->ET_RECEIPT_CATEGORY_TYPE; else echo "-"; ?></td>
								<td><?php if($result->EUC_CHEQUE_NO != NULL) echo $result->EUC_CHEQUE_NO; else echo "-"; ?></td>							
								<td><?php if($result->EUC_BANK_NAME != NULL) echo $result->EUC_BANK_NAME .", ". $result->EUC_BRANCH_NAME; else echo "-"; ?></td>
								<td><?php if($result->EUC_CHEQUE != Null) echo $result->EUC_CHEQUE; else echo $result->EUC_CHEQUE_DEPOSIT; ?></td>
								<td><?php echo date('d-m-Y',strtotime($result->EUC_DATE)); ?></td>

								<?php if(@$_SESSION['userGroup'] == "1" || @$_SESSION['userGroup'] == "6" || @$_SESSION['userGroup'] == "2" ){ ?><td style="text-align:center;">
									<a style="margin-left:9px;" onclick="editEOD('<?=$result->ET_RECEIPT_CATEGORY_TYPE; ?>','<?=$result->EUC_CHEQUE_NO; ?>','<?=$result->EUC_CHEQUE_DATE; ?>','<?php echo str_replace("'","\'",$result->EUC_BANK_NAME);?>','<?php echo str_replace("'","\'",$result->EUC_BRANCH_NAME);?>','<?=$result->EUC_CHEQUE; ?>','<?=$result->EUC_ID; ?>','<?=$result->EUC_EOD_DATE; ?>','<?=$result->ET_RECEIPT_ID; ?>')" title="Bank Deposit"><img style="margin-right:4px;width:24px; height:24px" src="<?=site_url();?>images/edit_icon.svg"/></a>
								</td><?php } ?>
							</tr>
						<?php } ?>
					</tbody>
...
<form id="submitForm" action="<?php echo site_url(); ?>EventEOD_Tally/eodTallySave" class="form-group" role="form" enctype="multipart/form-data" method="post">
	...
	<input type="hidden" id="EUC_CHEQUE_NO" name="EUC_CHEQUE_NO">
	<input type="hidden" id="EUC_CHEQUE_DATE" name="EUC_CHEQUE_DATE">
	<input type="hidden" id="EUC_BANK_NAME" name="EUC_BANK_NAME">
	<input type="hidden" id="EUC_BRANCH_NAME" name="EUC_BRANCH_NAME">
	<input type="hidden" id="depositdate2" name="depositdate">
	<input type="hidden" id="isDeposited" name="isDeposited">
	<input type="hidden" id="updateId" name="updateId">
	<input type="hidden" id="ET_RECEIPT_ID" name="ET_RECEIPT_ID">
</form>
<script>

	function editEOD(ET_RECEIPT_CATEGORY_TYPE,EUC_CHEQUE_NO,EUC_CHEQUE_DATE,EUC_BANK_NAME,EUC_BRANCH_NAME,EUC_CHEQUE,EUC_ID,EUC_EOD_DATE,ET_RECEIPT_ID) {

		let url = "<?=site_url()?>EventEOD_Tally/checkPreviousPendingDate/";
		$.post(url, {
			date:EUC_EOD_DATE
		},function(e) {
			if(e == "success") {
