			...
			<table class="table table-bordered table-hover">
				...
				<tbody>
					<?php foreach($indChequeDetail as $result) { ?>
						<tr>
							<td><center><?php echo $result->CHEQUE_NO; ?></center></td>
							<?php if($result->FCD_STATUS == "Cancelled") { 
							$cancelNotes = str_replace("'","\'",$result->CHEQUE_CANCELLED_NOTES);?>
							<td><a class="log mymodelcancel" style="color:#800000;" onclick="show_cancelled('<?php echo $cancelNotes; ?>')"><?php echo $result->FCD_STATUS; ?></a></td>
							<?php } else { ?>
								<td><?php echo $result->FCD_STATUS; ?></td>
							<?php } ?>
							<td><center><?php echo $result->FLT_DATE; ?></center></td>
							<td><?php echo $result->FAVOURING_NAME; ?></td>
							<td><?php echo $result->VOUCHER_NO; ?></td>
							<td><center><?php echo $result->CHEQUE_DATE; ?></center></td>
							<td><center><?php echo $result->RECONCILED_DATE; ?></center></td>	
							<td>
								<?php if($result->FCD_STATUS == "Unreconciled" || $result->FCD_STATUS == "Available") { ?>
									<center><a  style="margin-left: 5px; " style="border:none; outline:0;"  title="Cancel Cheque" onClick="callCancelModal('<?=$result->CHEQUE_NO; ?>','<?=$result->FCD_STATUS; ?>','<?=$result->FCD_ID;?>','<?=$result->VOUCHER_NO;?>','<?=$result->FCBD_ID;?>');"><img style="border:none; outline: 0; width:24px; height:24px" src="<?php echo base_url();?>images/delete.svg"></a></center>
								<?php } ?>
								<?php if($result->FCD_STATUS == "Cancelled" && $result->FLT_DATE == "")  { ?>
									<center><a  style="margin-left: 5px; " style="border:none; outline:0;"  title="Activate Cheque" onClick="ActivateCancelledCheque('<?=$result->FCD_ID;?>','<?=$result->FCBD_ID;?>');"><img style="border:none; outline: 0; width:24px; height:24px" src="<?php echo base_url();?>images/check_icon.svg"></a></center>
								<?php } ?>
							</td>
							<td>
								<?php if($result->FCD_STATUS == "Available" || $result->FCD_STATUS == "Unreconciled"  ) { ?>
                    			<center><a  style="margin-left: 5px; " style="border:none; outline:0;"  title="MultiPayment" onClick="MultiPaymentCheque('<?=$result->CHEQUE_NO; ?>','<?=$result->FCD_STATUS; ?>','<?=$result->FCBD_ID;?>');"><img style="border:none; outline: 0; width:24px; height:24px" src="<?php echo base_url();?>images/duplicate.svg"></a> 
								<?php } ?>
								<?php if($result->FCD_STATUS == "Available" && $result->IS_MULTI_PAYMENT == 1) { ?>
                    			<a  style="margin-left: 5px; " style="border:none; outline:0;"  title="Delete MultiPayment Cheque" onClick="DeleteMultiPaymentCheque('<?=$result->FCD_ID;?>');"><img style="border:none; outline: 0; width:24px; height:24px" src="<?php echo base_url();?>images/trash.svg"></a> </center>
								<?php } ?>
							</td>
...
<div id="myModalCancelled" class="modal fade" role="dialog">
  ...
  <form id="submitMultiPaymentForm" action="<?php echo site_url(); ?>finance/multipaymentCheque" class="form-group" role="form" enctype="multipart/form-data" method="post">	
  ...
  <form id="deletesubmitMultiPaymentForm" action="<?php echo site_url(); ?>finance/deletemultipaymentCheque" class="form-group" role="form" enctype="multipart/form-data" method="post">	
...
<script type="text/javascript">
    ...
    function callCancelModal(CHEQUE_NO,FCD_STATUS,FCD_ID,VOUCHER_NO,FCBD_ID) {
	...
	function ActivateCancelledCheque(FCD_ID,FCBD_ID){
		$('#fcdid').val(FCD_ID);
		$('#fcbdid').val(FCBD_ID);
		$('#activateCancelledChequeForm').attr('action','<?=site_url()?>finance/activateCancelledCheque');
