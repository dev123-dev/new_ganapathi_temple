...
<div class="row form-group">							
	<div class="col-lg-3 col-md-10 col-sm-10 col-xs-8" style = "padding-right:0px;padding-top:10px;">
	 ...
	 <div class="col-lg-3 col-md-10 col-sm-10 col-xs-8">
        <form id="frmCommitteeChange" action="<?=site_url()?>finance/displayBalanceSheet" method="post">
...
<form id="frmOpenBalSheet" action="<?=site_url()?>finance/displayBalanceSheet" method="post" >
...
<div class="col-lg-12 col-md-12 col-sm-8 col-xs-8 pull-right text-right" style = "padding-right:0px;padding-bottom:10px;padding-top:10px; margin-top:-4em">
	<a style="text-decoration:none;cursor:pointer;margin-right: -30px" href="<?=site_url()?>finance/displayBalanceSheet" title="Refresh"><img style="width:24px; height:24px" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
						 ...
						 if($row->LEVELS =='LG'){ ?>
							...
							<?php foreach($Cash as $row2) {
									if($row->FGLH_ID == $row2->FGLH_ID && $row2->CASH>0) {?> 
										...
										<td><a class="breakup" style="text-decoration:none;cursor: pointer;color:#ca7933 !important;" onClick="breakupDetails('<?=$row2->FGLH_ID; ?>','<?php echo str_replace("'","\'",$row2->FGLH_NAME);?>','<?=$row2->COMP_ID; ?>')">&emsp;&emsp;-><?php echo $row2->COMP_NAME; ?></td>
			...
			<section> 
				<table class="table"  >
					...
					foreach($assets as $row) {
						if($row->LEVELS == 'SG' || $row->LEVELS == 'PSG'){
							$trail = $row->FGLH_ID;
							$y=1;
							$some=0;
...
let frommin =  $('#fromBsDate').val();
 ...
 let minmaxYear =Financeyear.split("-"); 
   ...
   let maxYear = Number(minYear) + 1;
	...
	function breakupDetails(FGLH_ID,FGLH_NAME,COMP_ID=""){
		$('#FGLH_ID').val(FGLH_ID);
		...
		$('#breakupDetailForm').attr('action','<?=site_url()?>finance/ledgerSummaryDetail');
