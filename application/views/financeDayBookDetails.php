	...
	<div class="row form-group">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						...
						<?php foreach($dayBookDetail as $result) { ?>
							<tr>
								<?php if($result->Account == 'From:') { ?> 
									<td><?php echo  $result->Account . '&nbsp;&nbsp;&nbsp;&nbsp;' . $result->FGLH_NAME;  ?></td>
									<input type="hidden" name="fromfghlh" id="fromfghlh" value="<?php echo $result->FGLH_NAME; ?>">
								<?php } else { ?>
									<td><?php echo $result->Account . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . $result->FGLH_NAME;  ?></td>
									<input type="hidden" name="tofghlh" id="tofghlh" value="<?php echo $result->FGLH_NAME; ?>">
								<?php } ?>
								<td align="right"></style><?php echo $result->FLT_DR; ?></td>
    		...
    		<div style="clear:both;" class="form-group">
	    		<button style="width:10%;font-weight: bold;" type="button" id="print" onClick="printdayBookDetail('<?=$result->FGLH_ID; ?>','<?php echo str_replace("'","\'",$result->FGLH_NAME);?>','<?=$result->VOUCHER_NO; ?>','<?=$result->VOUCHER_TYPE; ?>','<?=$result->FLT_DATE; ?>','<?=$chequeno; ?>','<?=$result->FLT_DR; ?>','<?=$naration; ?>','<?=$FLT_DATE; ?>','<?=$result->FLT_CR; ?>','<?=$chequedate; ?>','<?=$result->PAYMENT_METHOD; ?>','<?=$result->RECEIPT_FAVOURING_NAME?>','<?=$result->BANK_NAME?>')" class="btn btn-default"> <span class="glyphicon glyphicon-print"></span> Print</button>
				...
				<?php if(($this->session->userdata('userGroup') == 1 || $this->session->userdata('userGroup') == 6) && $dayBookDetail[0]->RECEIPT_ID==0 && !($dayBookDetail[0]->RP_TYPE=="C1" && $dayBookDetail[0]->FGLH_ID == 21) && $dayBookDetail[0]->RP_TYPE!="OP" && $dayBookDetail[0]->TRANSACTION_STATUS != "Cancelled") { ?>
	    			<button style="font-weight: bold;float: right;" type="button" id="print" onClick="cancelTransaction('<?=$result->VOUCHER_NO; ?>','<?=$FLT_DATE; ?>','<?=$chequeno; ?>','<?=$voucherType ?>')" class="btn btn-default"><span class="glyphicon glyphicon-remove-circle"></span> Cancel Transaction</button>
	    		<?php } ?>
	    	</div>
    </div>
		</div>
...
<script>
	...
	function cancelTransaction(VOUCHER_NO,FLT_DATE,chequeno,voucherType){
		...
		$('#cancelTransactionForm').attr('action','<?=site_url()?>finance/cancelTransaction');
		$('#cancelTransactionForm').submit();
	}


	function printdayBookDetail(FGLH_ID,FGLH_NAME,VOUCHER_NO,VOUCHER_TYPE,FLT_DATE,CHEQUE_NO,FLT_DR,naration,FLT_DATE,FLT_CR,chequedate,PAYMENT_METHOD,RECEIPT_FAVOURING_NAME,BANK_NAME) {
	   	...
	   	let RECEIPTFAVOURINGNAME = RECEIPT_FAVOURING_NAME;
	    ...
	    if( VOUCHER_TYPE =='Receipt' || VOUCHER_TYPE =='Journal' || VOUCHER_TYPE =='Contra'){
			...
			$.post(url,{'VOU_CHER_TYPE':VOUCHERTYPE,'FGLH_NAME':FGLH_NAME,'VOUCHER_NUMBER':vouchernumber,'CHEQUE_NO':chkno,'naration':narration,'FLT_DATE':FLTDATE,'FLTDR':FLT_DR_AMT,'TONAME':tofghlh,'FROMNAME':fromfghlh,'chequedate':chkdate,'RECEIPT_FAVOURING_NAME':RECEIPTFAVOURINGNAME,'BANK_NAME':bankname }, function(data) {	
			...
			$.post(url,{'VOU_CHER_TYPE':VOUCHERTYPE,'FGLH_NAME':FGLH_NAME,'VOUCHER_NUMBER':vouchernumber,'CHEQUE_NO':chkno,'naration':narration,'FLT_DATE':FLTDATE,'FLT_CR':FLT_CR_AMT,'TONAME':tofghlh,'FROMNAME':fromfghlh,'chequedate':chkdate,'PAYMENT_METHOD':PAYMENTMETHOD,'RECEIPT_FAVOURING_NAME':RECEIPTFAVOURINGNAME,'BANK_NAME':bankname }, function(data) {
