...
<div class="col-lg-12 col-md-12 col-sm-8 col-xs-8 pull-right text-right" style = "padding-right:0px;padding-bottom:10px;padding-top:10px; margin-top:-4em">
	<a style="text-decoration:none;cursor:pointer;pull-right;" href="<?=site_url()?>finance/allLedgerTranscation" title="Refresh"><img style="width:24px; height:24px" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
				...
				<tbody>
					<?php foreach($dayBook as $result) { ?>
						<tr>
							...
							<td><a style="text-decoration:none;cursor:pointer;color:#800000;" onClick="dayBookDetail('<?=$result->FGLH_ID; ?>','<?php echo str_replace("'","\'",$result->FGLH_NAME);?>','<?=$result->VOUCHER_NO; ?>','<?=$result->FLT_DATE; ?>')"><?php echo $result->FGLH_NAME; ?></a></td>
...
<script>
	...
	function get_datefield_change(date) {
		...
		url = "<?php echo site_url(); ?>finance/all_ledger_transcation_on_change_date";
	...
	$('#EveryRadio').on('click', function() {
		...
		url = "<?php echo site_url(); ?>finance/allLedgerTranscation";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	});
