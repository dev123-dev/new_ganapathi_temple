<div class="container">
	...
	<div class="row form-group">
		<div class="col-lg-10 col-md-10 col-sm-10 col-xs-8">
			<span class="eventsFont2"><?=$templename[0]["TRUST_NAME"]?> Day Book</span>
		</div>
		<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">
			<a style="width:24px; height:24px" class="pull-right img-responsive" href="<?=site_url()?>/TrustReport/trust_day_book" title="Reset"><img title="reset" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
	</div>
			...
			<div class="multiDate">
				<div class="col-lg-2 col-md-2 col-sm-4 col-xs-5" style="margin-bottom:1em;">
					<div class="input-group input-group-sm">
						<input id="todayDate" name="todayDate" type="text" value="<?php echo $date; ?>" class="form-control todayDate" placeholder="dd-mm-yyyy" onchange="get_datefield_change(this.value)" autocomplete="off" readonly="readonly">
			...
			<div class="EveryRadio" style="display:none;">
				<div class="col-lg-2 col-md-2 col-sm-3 col-xs-6">
					<div class="form-group">
						<div class="input-group input-group-sm">
							<input name="fromDate" onchange="get_datefield_change(this.value)" id="fromDate" type="text" class="form-control fromDate2" value="<?=@$fromDate; ?>" placeholder="From: dd-mm-yyyy" autocomplete="off" readonly="readonly"/>
							<div class="input-group-btn">
				...
				<div class="col-lg-2 col-md-2 col-sm-3 col-xs-6">
					<div class="form-group">
						<div class="input-group input-group-sm">
							<input name="toDate" onchange="get_datefield_change(this.value)" id="toDate" type="text" value="<?=@$toDate; ?>" class="form-control toDate2" placeholder="To: dd-mm-yyyy" autocomplete="off" readonly="readonly"/>
							<div class="input-group-btn">
...
<div class="container">
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<table class="table table-bordered table-hover">
				...
				<tbody>
					<?php foreach($trust_day_book as $result) { 
						if($result->qty != ''){
							$amt = intval($result->amt)*intval($result->qty);
							$total =intval($result->total)*intval($result->qty);
						}else{
							$amt = $result->amt;
							$total = $result->total;
						}
						...
						<tr class="row1">
							<?php if($result->rType != "") { ?>
								...
								<?php } else { ?>
									<?php if($result->receiptFor == "Trust" || $result->receiptFor == "Trust (Hall)") { ?>
	...
	<div class= "row">
		<ul class="pagination pagination-sm" style="margin-left:15px;margin-top: -0.1em;">
			<?=$pages; ?>
		</ul> 				
	</div> 
</div>
...
<form id="deityForm" action="<?=site_url()?>TrustReceipt/receipt_newTrustPrint" method="post">
	<input type="hidden" id="receiptFormat2deity" name="receiptFormat" />
	<input type="hidden" id="receiptId2" name="receiptId" />
</form>
...
<script>	
	...
	function printReceiptDeity(receiptFormat,receiptId,catId) {
		if(catId == 1) {
			...
			$('#receiptId2').val(receiptId);
			$('#deityForm').submit();
		} else if(catId == 2) {
			$('#receiptIdDeity').val(receiptId);
			$('#receiptFormat1Deity').val(receiptFormat);
			$('#receiptFormDeity').attr('action','<?=site_url()?>TrustReceipt/receipt_newTrustPrint');
	...
	function GetSendField() {
		...
		url = "<?php echo site_url(); ?>TrustReport/trust_day_book_excel";
	...
	function get_datefield_change(date) {
		...
		document.getElementById('paymentMethod').value = $('#modeOfPayment').val();
		document.getElementById('deityId').value = $('#deity').val();
		url = "<?php echo site_url(); ?>TrustReport/trust_day_book_on_change_date";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	}
	...
	$('#EveryRadio').on('click', function() {
		...
		url = "<?php echo site_url(); ?>TrustReport/trust_day_book";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	});
