<div class="container">
	...
	<div class="row form-group">
		<div class="col-lg-10 col-md-10 col-sm-10 col-xs-10">
			<span class="eventsFont2">Current Event Receipt Report: <span class="samFont"><?php echo $events[0]->TET_NAME; ?></span></span>
		</div>
		<div class="col-lg-2 col-md-2 col-sm-2 col-xs-2" style="margin-top: 1em;">
			<a style="width:24px; height:24px" class="pull-right img-responsive" href="<?=site_url()?>TrustReport/event_receipt_report" title="Refresh"><img title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
	</div>
	...
	<div class="row form-group">
			...
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-9" style="margin-bottom:1em;display:none;">
			  <select id="deity" name="deity" class="form-control" disabled>
				<?php foreach($events as $result) { ?>
					<option value="<?php echo $result->TET_ID; ?>"><?php echo $result->TET_NAME; ?></option>
				<?php } ?>
			  </select>
			</div>
		<!-- code from abhishek -->
		...
		<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
				<select id="users" name="users" class="form-control" onchange="GetDataOnUser(this.value,'<?php echo site_url()?>TrustReport/event_report_on_change_date')">
			...
			<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
				<div class="form-inline">
					...
					foreach($TotalAmount as $result) { 
						if($result->TET_RECEIPT_ACTIVE == 1) {
							$amount = (int)($amount) + (int)($result->TET_RECEIPT_PRICE) + (int)($result->POSTAGE_PRICE);
						}
					} ?>
				...
				<table class="table table-bordered table-hover">
					<thead>
					  <tr>
						...
						<th>Total</th>
						<th>Status</th>
						<th style="width:5%">Payment Notes</th>
						...
						<?php foreach($event_receipt_report as $result) { ?>
							<tr class="row1">
								<?php if($result->PAYMENT_STATUS == "Cancelled") { ?>
									<td style="text-decoration:line-through;"> <?php echo $result->TET_RECEIPT_NO; ?></td>
								<?php } else { ?>
								...
								<?php } ?>
								<td style="width:20%;word-break: break-all;"><?php echo $result->TET_RECEIPT_PAYMENT_METHOD_NOTES; ?></td>
	...
	<div class="row form-group">
		<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
			...
			<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12" style="font-size:18px;margin-top: -1em;">
				<label class="pull-right" for="sevaAmount">Total Receipt:<strong style="font-size:18px;"></strong> <span id="totalSeva" style="margin-right:15px"><?php echo $Count; ?></span></label>
...
function GetDataOnUser(users,url) {
	...
	function GetSendField() {
		...
		url = "<?php echo site_url(); ?>TrustReport/event_receipt_report_excel";
	...
	function generatePDF() {
		...
		document.getElementById('payMode').value = $('#modeOfPayment').val();
		document.getElementById('dateField').value = $('#todayDate').val();
		url = "<?php echo site_url(); ?>generatePDF/create_trustEventReceiptpdf";
		$("#report").attr("action",url)
		$("#report").submit();
	}
	...
	function print() {
		...
		let url = "<?php echo site_url(); ?>generatePDF/create_trustEventReceiptSession";
		...
		$.post(url,{'payMode':$('#modeOfPayment').val(),'dateField':$('#todayDate').val(),'radioOpt':radioOpt, 'allDates':allDates}, function(data) {
			let url2 = "<?php echo site_url(); ?>generatePDF/create_trustEventReceiptPrint";
			if(data == 1) {
					downloadClicked = 0;
					var win = window.open(
					  url2,
	...
	function get_datefield_change(date) {
		...
		document.getElementById('paymentMethod').value = $('#modeOfPayment').val();
		url = "<?php echo site_url(); ?>TrustReport/event_report_on_change_date";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	}
	...
	$('#EveryRadio').on('click', function() {
		...
		url = "<?php echo site_url(); ?>TrustReport/event_receipt_report";
		$("#report").attr("action",url)
		$("#report").submit();
	});
