	...
	<page style="margin-top:25px;margin-left:75%;width:115%;margin-right:75%;margin-height:auto;">
		<form>	
			...
			<?php if($result["RECEIPT_PAYMENT_METHOD"] == "Cheque") { ?>
				<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Cheque Number&nbsp;</strong>: <?=$result['CHEQUE_NO'] ?></span>
				<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Cheque Date&nbsp;</strong>: <?=$result['CHEQUE_DATE'] ?></span><br/>
				<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Bank&nbsp;</strong>: <?=$result['BANK_NAME'] ?></span>
				<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Branch&nbsp;</strong>: <?=$result['BRANCH_NAME'] ?></span><br/>
			<?php } else if($result["RECEIPT_PAYMENT_METHOD"] == "Credit / Debit Card") { ?>
				<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Transaction Id&nbsp;&thinsp;</strong>: <?=$result['TRANSACTION_ID'] ?></span><br/>
			<?php }  ?>
			<br/><!-- <br/> -->
...
<?php global $t; $t = 0; foreach($hallBookingTemple as $result) { $t++; ?>
	<page style="margin-top:25px;margin-left:75%;width:115%;margin-right:75%;margin-height:auto;">
		<form>
			...
			<div style="margin-bottom:6px;"><!--This is required for correct spacing do not remove--></div>
			<span style="font-size:11px;letter-spacing:1px;"><strong>Deity&emsp;&ensp;&ensp;&nbsp;&thinsp;: <?=$result['RECEIPT_DEITY_NAME'] ?></strong></span><br/>
			<div style="margin-bottom:6px;"><!--This is required for correct spacing do not remove--></div>
			<span style="font-size:11px;letter-spacing:1px;"><strong>Name&emsp;&ensp;&ensp;&nbsp;: <?=$result['RECEIPT_NAME']; ?></strong></span><br/>
			<span style="font-size:11px;letter-spacing:1px;"><strong>Number&nbsp;&nbsp;&nbsp;&nbsp;: </strong><?=$result['RECEIPT_PHONE'] ?></span><br/>
			<span style="font-size:11px;letter-spacing:1px;word-wrap: break-word;"><strong>Address&nbsp;&nbsp;&nbsp;&nbsp;: </strong><?=$result['RECEIPT_ADDRESS'] ?></span><br/>
			<div style="margin-bottom:6px;"><!--This is required for correct spacing do not remove--></div>
			<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Amount&nbsp;&nbsp;: Rs. <?=$result['RECEIPT_PRICE'] ?>/- <?=AmountInWords($result['RECEIPT_PRICE']);?></strong></span><br/>
			<div style="margin-bottom:6px;"><!--This is required for correct spacing do not remove--></div>
			<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Payment Mode&nbsp;</strong>: <?=$result['RECEIPT_PAYMENT_METHOD'] ?></span><br/>
			<?php if($result["RECEIPT_PAYMENT_METHOD"] == "Cheque") { ?>
							<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Cheque No&nbsp;</strong>: <?=$result['CHEQUE_NO'] ?></span>
							<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Cheque Date&nbsp;</strong>: <?=$result['CHEQUE_DATE'] ?></span><br/>
							<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Bank&nbsp;</strong>: <?=$result['BANK_NAME'] ?></span>
							<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Branch&nbsp;</strong>: <?=$result['BRANCH_NAME'] ?></span><br/>
							<?php } else if($result["RECEIPT_PAYMENT_METHOD"] == "Credit / Debit Card") { ?>
							<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2"><strong>Transaction Id&nbsp;</strong>: <?=$result['TRANSACTION_ID'] ?></span><br/>
							<?php }  ?>
			<br/><!-- <br/> -->
...
<div class="container">
	...
	<div class="row" style="width:105.6%">
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12" style="margin-left: -17px;">
			<div class="clear:both;table-responsive">
				<table id="eventSeva" class="table table-bordered table-hover">
					<thead>
					<tr><td colspan=5 style="font-size:16px;"><center>Trust Receipt</center></td></tr>
					<tr>
						<th>Receipt No</th>
						<th>Name</th>
						<th>Amount</th>
					...
					<?php 
						...
						foreach($hallBookingTrust as $result) {
							...
							echo "<td>". $result["TR_PAYMENT_METHOD_NOTES"]."</td></tr>";
						}
					?>
					</tbody>
				</table>
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		  <div class="clear:both;table-responsive">
			<table id="eventSeva" class="table table-bordered table-hover">
				<thead>
				<tr><td colspan=5 style="font-size:16px;"><center>Temple Receipt</center></td></tr>
				  <tr>
				...
				<?php 
					...
					foreach($hallBookingTemple as $result) {
						...
						echo "<td>". $result["RECEIPT_PAYMENT_METHOD_NOTES"]."</td></tr>";
					}
				?>
				</tbody>
			</table>
	...
	<div class="form-group">
		<?php if($PAYMENT_STATUS == "noPayment") { ?>
		<center><button onclick="location.href='<?=site_url(); ?>Trust'" type="button" class="btn btn-default btn-lg">All Hall Booking</button></center>	
	<?php } else { ?>
...
var receiptId = "<?=@$hallBookingTemple[0]['RECEIPT_ID']; ?>"
	...
	//for receipts from trust head
	if('<?php echo $hallBookingTrust[0]['PRINT_STATUS']?>' == 1)
		$('#print').html(" Re-Print Receipt");
	
	//for receipts from temple head (not required much but kept for safety)
	if('<?php echo $hallBookingTemple[0]['PRINT_STATUS']?>' == 1)
		$('#print').html(" Re-Print Receipt");
	
	
	//these lines are to display duplictae & Hall receipt
	if('<?php echo $hallBookingTrust[0]['PRINT_STATUS']?>' == 1)
			$('#duplicate').show();
		else{
		var i;
	...
	var duplicate = 0; //required for first iteration of printing 
