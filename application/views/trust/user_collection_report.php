<div class="container">
	...
	<div class="row form-group">
		...
		<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">
			<a style="width:24px; height:24px" class="pull-right img-responsive" href="<?=site_url()?>TrustReport/user_collection_report" title="Reset"><img title="reset" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
	</div>
	...
	<div class="row form-group">
		<form id="userFilters" enctype="multipart/form-data" method="post" accept-charset="utf-8">
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
			  <select id="modeOfPayment" name="modeOfPayment" class="form-control" onchange="GetDataOnFilter(this.value,'<?php echo site_url()?>TrustReport/get_data_on_payment')">
						...
						<?php foreach($event_receipt_report as $result) { ?>
							<tr class="row1">
								...
								<?php if($result->TET_RECEIPT_PAYMENT_METHOD == "Cheque") { ?>
									<td><a class="log mymodelcancel" style="color:#800000;" onclick="show_cheque('1','<?php echo $result->CHEQUE_NO ; ?>','<?php echo $result->CHEQUE_DATE; ?>','<?php echo str_replace("'","\'",$result->BANK_NAME); ?>','<?php echo str_replace("'","\'",$result->BRANCH_NAME); ?>','<?php echo $result->TRANSACTION_ID; ?>')"><?php echo $result->TET_RECEIPT_PAYMENT_METHOD; ?></a></td> 
...
<script>
	...
	function GetSendField() {
		...
		url = "<?php echo site_url(); ?>TrustReport/event_sevas_report_excel";
	...
	function get_datefield_change(date) {
		...
		url = "<?php echo site_url(); ?>TrustReport/event_date_change_report";
	...
	function get_seva_change(sevaId) {
		...
		url = "<?php echo site_url(); ?>TrustReport/event_seva_change_report";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	}
