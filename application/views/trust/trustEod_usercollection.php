	...
	<div class="row form-group">
		...
		<form id="approveId" enctype="multipart/form-data" method="post" accept-charset="utf-8">
			...
			<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">			
				...
				<a class="pull-right" onclick="GetRefresh('<?=site_url()?>TrustEOD/trustEod_usercollection')" title="Refresh"><img style="width:24px; height:24px;margin-right:20px;cursor:pointer" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
	...
	<div class="row form-group">
		<form id="userFilters" enctype="multipart/form-data" method="post" accept-charset="utf-8">
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
			  <select id="modeOfPayment" name="modeOfPayment" class="form-control" onchange="GetDataOnFilter(this.value,'<?php echo site_url()?>TrustEOD/get_data_on_filter/1','<?php echo $UserDate ?>','<?php echo $UserName ?>','<?php echo $UserId ?>')">
				...
				<table class="table table-bordered table-hover">
					...
					<tbody>
						<?php $selectedId = ""; 
							...
							<tr class="row1">
								<?php $selectedId .= $result->TR_ID .",";  ?>
								<!--HIDDEN FIELD USED FOR UNCHECK CHECKBOX-->
								<input type="hidden" name="pageVal" id="pageVal"/>
								<?php if(isset($_SESSION['all_users'])) { ?>
									<td><center><input type="checkbox" name="<?php echo $result->TR_ID; ?>" id="<?php echo $result->TR_ID; ?>" onchange="GetUnselect('<?php echo $result->TR_ID; ?>')" class="sel" checked /></center></td>
								<?php } else { ?>
									<td><center><input type="checkbox" name="<?php echo $result->TR_ID; ?>" id="<?php echo $result->TR_ID; ?>" onchange="GetUnselect('<?php echo $result->TR_ID; ?>')" class="sel"/></center></td>
								<?php } ?>
...
<script>
	...
	function GetUnselect(receiptId) {
		...
		if($('#all_users').prop('checked') == true) {
			...
			url = "<?php echo site_url(); ?>TrustEOD/get_unset_session";
			...
			if(count == inputs.length) {
				...
				url = "<?php echo site_url(); ?>TrustEOD/get_set_session";
	...
	function approveSubmit(date,username,uid) {
		...
		$('#userIdChk').val(uid);
		url = "<?php echo site_url(); ?>TrustEOD/approve_Submit";
	...
	function show_cheque(id,cheqNo,cheqDate,Bank,Branch,transactionId){
        var c_url ="<?php echo site_url(); ?>TrustEOD/View";
        $.ajax({
			url: c_url,
