<div class="container">
	...
	<div class="row form-group">
		<div class="col-lg-10 col-md-10 col-sm-10 col-xs-8">
			<span class="eventsFont2">Current Event Sevas Report: <span class="samFont"><?php echo $events[0]->TET_NAME; ?></span></span>
		</div>
		<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">
			<a style="width:24px; height:24px" class="pull-right img-responsive" href="<?=site_url()?>TrustReport/event_seva_report" title="Refresh"><img title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
	</div>
			...
			<div class="col-lg-2 col-md-3 col-sm-3 col-xs-9">
			  ...
			  <select id="sevas" name="sevas" class="form-control" onchange="get_seva_change(this.value)">
				...
				<?php foreach($events_seva as $result) { ?>
					<?php if($sevaId == $result->TET_SO_SEVA_ID) { ?>
						<option selected value="<?php echo $result->TET_SO_SEVA_ID; ?>"><?php echo $result->TET_SO_SEVA_NAME; ?></option>
					<?php } else { ?>
						<option value="<?php echo $result->TET_SO_SEVA_ID; ?>"><?php echo $result->TET_SO_SEVA_NAME; ?></option>
					<?php } ?>
				<?php } ?>
			  </select>
			</div>
				...
				<table class="table table-bordered table-hover">
					...
					<tbody>
						<?php foreach($seva_report as $result) { ?>
							<tr class="row1">
								<td><?php echo $result->TET_SO_SEVA_NAME; ?></td>
...
<script>
	...
	$('#EveryRadio').on('click', function() {
		...
		document.getElementById('radioOptField').value = "multiDate";
		url = "<?php echo site_url(); ?>TrustReport/event_seva_report";
		$("#report").attr("action",url)
		$("#report").submit();
	});
	...
	function generateAddPDF() {
		...
		url = "<?php echo site_url(); ?>EventGenerationFPDF/trustEventAddress";
		$("#report").attr("action",url)
		$("#report").submit();
	}
	...
	function generatePDF() {
		...
		url = "<?php echo site_url(); ?>generatePDF/create_trustSevaReceiptpdf";
		$("#report").attr("action",url)
		$("#report").submit();
	}
	
	function print() {
		let url = "<?php echo site_url(); ?>generatePDF/create_trustSevaReceiptSession";
		...
		$.post(url,{'SId':$('#sevas').val(),'dateField':$('#todayDate').val(),'radioOpt':radioOpt, 'allDates':allDates}, function(data) {
			let url2 = "<?php echo site_url(); ?>generatePDF/create_trustSevaReceiptPrint";
			if(data == 1) {
					downloadClicked = 0;
					var win = window.open(
					  url2,
