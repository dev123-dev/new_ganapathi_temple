...
<div class="container-fluid container">
    <div class="row form-group">
        ...
        <div class="col-lg-3 col-md-10 col-sm-10 col-xs-8">
            <form id="frmCommitteeChange" action="<?=site_url()?>Trustfinance/Payment" method="post">
        ...
        <div class="col-lg-2 col-md-2 col-sm-2 col-xs-4" style = "padding-top:10px;">
            <a style="text-decoration:none;cursor:pointer;pull-right;" href="<?=site_url()?>Trustfinance/Payment" title="Refresh"><img style="width:24px; height:24px" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
        </div>
    </div>
    <form id="frmPayment" action="<?=site_url()?>Trustfinance/addPaymentTrans" method="post" enctype="multipart/form-data"
                ...
                <select id="lidP" class="form-control" name="lidP" style="height: 30px;width:100%" onChange="LedgerChange();">
                    ...
                    <?php   if(!empty($ledger)) {
                        foreach($ledger as $row1) { ?> 
                             <?php if($_SESSION['lidSes'] == $row1->T_FGLH_ID) { ?>
                                <option value="<?php echo $row1->T_FGLH_ID;?>|<?php echo $row1->BALANCE;?>|<?php echo $row1->T_FGLH_NAME;?>|<?php echo $row1->T_TYPE_ID;?>" selected><?php echo $row1->T_FGLH_NAME;?></option>
                            <?php } else { ?>
                                <option value="<?php echo $row1->T_FGLH_ID;?>|<?php echo $row1->BALANCE;?>|<?php echo $row1->T_FGLH_NAME;?>|<?php echo $row1->T_TYPE_ID;?>"><?php echo $row1->T_FGLH_NAME;?></option>
                            <?php } ?>
                        <?php } } ?>
                    </select>
                ...
                <select id="aidP"  class="form-control" name="aidP" style="height: 30px;width:100%" onChange="AccountChange();">
                    ...
                    <?php   if(!empty($account)) { 
                        foreach($account as $row1) { ?> 
                            <?php if($_SESSION['aidSes'] == $row1->T_FGLH_ID) { ?>
                                <option value="<?php echo $row1->T_FGLH_ID;?>|<?php echo $row1->BALANCE;?>|<?php echo $row1->T_FGLH_NAME;?>|<?php echo $row1->T_FGLH_PARENT_ID;?>|<?php echo str_replace("'","\'",$row1->T_BANK_NAME);?>|<?php echo str_replace("'","\'",$row1->T_BANK_BRANCH);?>" selected><?php echo $row1->T_FGLH_NAME;?></option>
                            <?php } else { ?>
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                              <?php foreach($pettyCashData as $result) { ?>
                                    <tr>
                                        <td><center><input type="checkbox" name="<?php echo $result->T_FGLH_ID; ?>" id="<?php echo $result->T_FGLH_ID; ?>" onchange="GetUnselect('<?php echo $result->T_FGLH_ID; ?>','<?php echo $result->AMOUNT; ?>')" class="sel" /></center></td>
                                        <td class="Bank"><?php echo $result->T_FGLH_NAME; ?></td>
                    ...
                    <div class="form-group col-lg-6 col-md-6 col-sm-6 col-xs-6">
                         <select id="aidC" class="form-control go" name="aidC" style="height: 30px;width:100%" onChange="AccountChange1();">
                            ...
                            <?php   if(!empty($account)) {
                                foreach($account as $row1) { 
                                    if( $row1->FGLH_ID != 27) {?> 
                                        <option value="<?php echo $row1->T_FGLH_ID;?>|<?php echo $row1->BALANCE;?>|<?php echo $row1->T_FGLH_NAME;?>|<?php echo $row1->T_FGLH_PARENT_ID;?>|<?php echo $row1->T_BANK_NAME;?>|<?php echo $row1->T_BANK_BRANCH;?>"><?php echo $row1->T_FGLH_NAME;?></option>
                            <?php } } } ?>
                        </select> 
...
<script type="text/javascript">
     function cheqfunc(){
         ...
         let bank_fglh_name =$('#FGLH_NAME').val();
          ...
          $('#multichequeform').attr('action','<?=site_url()?>Trustfinance/chequeDetailsPayment');
    ...
    function PrintSubmit(){
        ...
        let url = "<?=site_url()?>Trustfinance/addPaymentTrans";
        ...
        if (e1[0].trim() == "success"){
            ...
            let url2 = "<?php echo site_url(); ?>generatePDF/create_TrustPaymentPrint";
                if(data == 1) {
                    downloadClicked = 0;
                    var win = window.open(
                      url2,
                    ...
                    location.href = "<?=site_url();?>Trustfinance/Payment";
                }
            })
        }
        else
    ...
    function AccountChange(){
        ...
        for (let i = 0; i < arr.length; ++i) {
            if (arr[i]['T_FGLH_ID'] == acNo){
                $('#rid').append('<option value="' + arr[i]['T_CHEQUE_NO'] +"|" + arr[i]['T_FCD_ID'] + "|" + arr[i]['T_FCBD_ID'] +'">' + arr[i]['T_CHEQUE_NO'] + '</option>');
            }
        }
        if(acc1[3] == 8){
            if($('#selectedId').val()!=""){
    ...
    function validatePettycash()
    {
        ...
        if (countC != 0) {
            ...
            e1 = e.split("|")
            if (e1[0] == "success"){
                location.href = "<?=site_url();?>Trustfinance/Payment?unsetCall=No";
            }
            else
                alert("Something went wrong, Please try again after some time");
            });
