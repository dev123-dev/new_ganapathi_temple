...
<div class="row form-group">							
	<div class="col-lg-12 col-md-12 col-sm-8 col-xs-8" style = "padding-right:0px;padding-top:10px;">
		<h3 style="margin-top:0px">Cheque Books of <?php echo $T_FGLH_NAME ?></h3>
	</div>
</div>
...
<div class="col-lg-12 col-md-12 col-sm-8 col-xs-8 pull-right text-right" style = "padding-right:0px;padding-bottom:10px;padding-top:10px; margin-top:-4em">
	...
	<a style="text-decoration:none;cursor:pointer;pull-right; margin-left: 5px;" href="<?=site_url()?>Trustfinance/chequeDetails" title="Refresh"><img style="width:24px; height:24px" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
					...
					<?php $i=1;
					 foreach($chequeDetail as $result) { ?>
						<tr>
							...
							<td><center><a style="text-decoration:none;cursor:pointer;color:#800000;" onClick="indChequeDetail('<?=$result->T_FCBD_ID; ?>', '<?=$result->T_CHEQUE_BOOK_NAME; ?>')"><?php echo $result->T_CHEQUE_BOOK_NO; ?></a></center></td>
							<td><center><?php echo $result->T_FROM_NO; ?></center></td>
							<td><center><?php echo $result->T_TO_NO; ?></center></td>
							<td><center><?php echo $result->T_NO_OF_CHEQUE; ?></center></td>
							<td><?php echo $result->T_CHEQUE_BOOK_NAME; ?></td>
							<td><?php echo $result->T_FCBD_STATUS; ?></td>
							<td>
								<?php if($result->T_FCBD_STATUS == "Available") { ?>
									<center><a  style="margin-left: 5px; " style="border:none; outline:0;"  title="Activate Cheque Book" onClick="activateCheque('<?=$result->T_FCBD_ID; ?>')" ><img style="border:none; outline: 0; width:24px; height:24px" src="<?php echo base_url();?>images/check_icon.svg"></a></center>
								<?php } ?>
							</td>
						</tr>
					<?php $i++; 
...
<form id="frmAddChequeDetails" action="<?=site_url()?>Trustfinance/addAddChequeDetails" method="post">
	<!-- Modal -->
				...
				<div class="modal-body">
					<div class="container-fluid container" id="bankdetail"style="">
						...
						<div class="row form-group">
							...
							<input type="hidden" id="T_FGLH_ID"  name="T_FGLH_ID" value="<?php echo $T_FGLH_ID ?>">
...
<script>
	function indChequeDetail(T_FCBD_ID,T_CHEQUE_BOOK_NAME){
		$('#FCBD_ID').val(T_FCBD_ID);
		$('#CHEQUE_BOOK_NAME').val(T_CHEQUE_BOOK_NAME);
		$('#chequeDetailForm').attr('action','<?=site_url()?>Trustfinance/individualChequeDetails');
		$('#chequeDetailForm').submit();
	}

	function activateCheque(T_FCBD_ID){
		$('#fcbdid').val(T_FCBD_ID);
	
		$('#activateChequeForm').attr('action','<?=site_url()?>Trustfinance/activateChequeBook');
...
function isNumberKey(evt)
{
 ...
 function validateAddCheque() {
    ...
    let tono = $('#tono').val();
    let url = "<?=site_url()?>Trustfinance/addAddChequeDetails";
