...
<div class="container">
	...
	<div class="col-offset-lg-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 pull-right text-right" style = "padding-right:0px;padding-bottom:0px; margin-top:-3.7em">
			<a style="margin-left: 9px;pull-right;" href="<?=site_url()?>Trustfinance/Group" title="Add Group"><img style="width:24px; height:24px" src="<?=site_url();?>images/add_icon.svg"/></a>
	</div>
	<div class="row form-group">
				...
				<table class="table table-bordered table-hover">
					...
					<tr>
						...
						<?php foreach($parentDetails as $parentResult) { 
							if($parentResult->T_FGLH_ID == $row->T_FGLH_PARENT_ID){
								$parentName = $parentResult->T_FGLH_NAME;
								$parentLevel = $parentResult->T_LEVELS; ?>
							<?php }
						 } ?>

						<td class="text-center">				
							<a style="border:none; outline: 0;" href="#" title="Edit Group" ><img style="border:none; outline: 0;" 
								onclick="editGroups('<?=$row->T_FGLH_ID; ?>','<?php echo str_replace("'","\'",$row->T_FGLH_NAME);?>','<?=$parentName?>','<?=$parentLevel?>','<?=$row->T_FGLH_PARENT_ID; ?>','<?=$row->T_LEVELS; ?>')" src="<?php echo	base_url(); ?>images/edit_icon.svg"></a>
						</td>	
						<td class="text-center">
						<a style="border:none; outline: 0;" title="Delete Group" ><img style="border:none; outline: 0;" 
								onclick="deleteGroups('<?=$row->T_FGLH_ID; ?>','<?php echo str_replace("'","\'",$row->T_FGLH_NAME);?>','<?=$parentName?>','<?=$parentLevel?>','<?=$row->T_FGLH_PARENT_ID; ?>','<?=$row->T_LEVELS; ?>')" src="<?php echo	base_url(); ?>images/delete.svg"></a>	
						...
						<td class="text-center">	
							<a style="border:none; outline: 0;" title="Transfer Group" ><img style="border:none; outline: 0;" 
								onclick="transferGroup('<?=$row->T_FGLH_ID; ?>','<?php echo str_replace("'","\'",$row->T_FGLH_NAME);?>','<?=$parentName?>','<?=$parentLevel?>','<?=$row->T_FGLH_PARENT_ID; ?>','<?=$row->T_LEVELS; ?>')" src="<?php echo	base_url(); ?>images/transfer.svg"></a>
	...
	<div class="col-offset-lg-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 pull-right text-right" style = "padding-right:0px;padding-bottom:0px; margin-top:-3.7em">
			<a style="margin-left: 9px;pull-right;" href="<?=site_url()?>Trustfinance/Ledger" title="Add Ledger"><img style="width:24px; height:24px" src="<?=site_url();?>images/add_icon.svg"/></a>
			<a style="text-decoration:none;cursor:pointer;pull-right;" href="<?=site_url()?>Trustfinance/allGroupLedgerDetails" title="Refresh"><img style="width:24px; height:24px" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
	</div>
	<div class="row form-group">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="table-responsive">
				<table class="table table-bordered table-hover">
					...
					<tr>
						...
						<a style="border:none; outline: 0;" title="Edit Ledger" ><img style="border:none; outline: 0;" 
								onclick="editLedger('<?=$result->T_FGLH_ID; ?>','<?php echo str_replace("'","\'",$result->T_FGLH_NAME);?>','<?=$result->T_FGLH_PARENT_ID; ?>','<?=$result->T_IS_JOURNAL_STATUS; ?>','<?=$result->T_IS_TERMINAL; ?>','<?=$result->T_IS_FD_STATUS; ?>','<?=$result->T_FD_MATURITY_START_DATE; ?>','<?=$result->T_FD_MATURITY_END_DATE; ?>','<?=$result->T_FD_INTEREST_RATE; ?>')" src="<?php echo	base_url(); ?>images/edit_icon.svg"></a>
						</td> 

						<td class="text-center">	
							<a style="border:none; outline: 0;" title="Delete Ledger" ><img style="border:none; outline: 0;" 
								onclick="deleteLedger('<?=$result->T_FGLH_ID; ?>','<?php echo str_replace("'","\'",$result->T_FGLH_NAME);?>','<?=$result->T_FGLH_PARENT_ID; ?>','<?=$result->T_IS_JOURNAL_STATUS; ?>','<?=$result->T_IS_TERMINAL; ?>','<?=$result->T_BALANCE; ?>','<?=$result->T_OPBALANCE; ?>','<?=$result->T_CURRENT_BALANCE; ?>')" src="<?php echo	base_url(); ?>images/delete.svg"></a>
						</td>

						<td class="text-center">	
							<a style="border:none; outline: 0;" title="Transfer Ledger" ><img style="border:none; outline: 0;" 
								onclick="transferLedger('<?=$result->T_FGLH_ID; ?>','<?php echo str_replace("'","\'",$result->T_FGLH_NAME);?>','<?=$result->T_FGLH_PARENT_ID; ?>','<?=$result->T_TRANSACTION_AMT; ?>')" src="<?php echo	base_url(); ?>images/transfer.svg"></a>
						</td>
						<td class="text-center">
						<a style="border:none; outline: 0; text-decoration:none" title="View Ledger"  >
						<div class="glyphicon glyphicon-search" style="; outline: 0;" onclick="ViewLedger('<?=$result->T_FGLH_ID; ?>','<?php echo str_replace("'","\'",$result->T_FGLH_NAME);?>','<?=$result->T_FGLH_PARENT_ID; ?>')"></div></a>
...
<div class="modal fade" id="editGroupModal" role="dialog">
	<div class="modal-dialog">
		...
		<div class="modal-content" style="width: 70%">
			...
			<form id="editGroupsForm" method="post" action="<?php echo site_url();?>Trustfinance/updateGroup">
...
<div class="modal fade" id="deleteGroupModal" role="dialog">
	<div class="modal-dialog">
		...
		<div class="modal-content" style="width: 70%">
			...
			<form id="deleteGroupsForm" method="post" action="<?php echo site_url();?>Trustfinance/discardGroup">
				<div class="modal-body">
					<div class="form-group">
						<div style="clear:both;" class="form-group col-lg-3 col-md-6 col-sm-6 col-xs-6">
							<label for="inputLimit" style="margin-top: 5px;"  ><span style="font-weight:600;">Type:</span></label><span ></span>
...
<form id="deleteLedgerForm" method="post" action="<?php echo site_url();?>Trustfinance/deleteLedger">
...
<form id="ViewLedger" method="post" action="<?php echo site_url();?>Trustfinance/ViewLedger">
