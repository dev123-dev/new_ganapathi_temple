...
<div class="container">
	...
	<div class="row form-group">							
        ...
        <form id="frmCommitteeChange" action="<?=site_url()?>Trustfinance/displayReceiptAndPayment" method="post">
	...
	<form id="frmOpenReceiptPayment" action="<?=site_url()?>Trustfinance/displayReceiptAndPayment" method="post" >
	...
	<div class="col-lg-12 col-md-12 col-sm-8 col-xs-8 pull-right text-right" style = "padding-right:0px;padding-bottom:10px;padding-top:10px; margin-top:-4em">
		<a style="text-decoration:none;cursor:pointer;margin-right: -30px" href="<?=site_url()?>Trustfinance/displayReceiptAndPayment" title="Refresh"><img style="width:24px; height:24px" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
	</div>
				...
				<div class="col-lg-6 col-md-6 col-xs-12 col-sm-12">
					<section> 
						<table class="table" style="border:0px;">
							...
							foreach($receipt as $Journal){
								if (!isset($Journal->LEVELS)) {
									$JournalSum +=$Journal->PBalanceCR;
							}
						}
									...
									if($row->T_LEVELS =='LG'){ ?>
										...
										<?php foreach($Cash as $row2) {
											if($row->T_FGLH_ID == $row2->T_FGLH_ID && $row2->CASH>0) {?> 
												...
												<td><a class="breakup" style="text-decoration:none;cursor: pointer;color:#ca7933 !important;" onClick="recPaybreakupDetails('<?=$row2->T_FGLH_ID; ?>','<?php echo str_replace("'","\'",$row2->T_FGLH_NAME);?>','<?=$row2->T_COMP_ID; ?>')">&emsp;&emsp;-><?php echo $row2->T_COMP_NAME; ?></td>
				...
				<div class="col-lg-6 col-md-6 col-xs-12 col-sm-12">
					<section> 
						<table class="table" style="border:0px;">
							...
							foreach($payment as $Journal){
								if (!isset($Journal->LEVELS)) {
									
									$journalSum+=$Journal->PBalanceDR;
							}
						}
...
let frommin =  $('#fromRP').val();
 ...
 let minmaxYear =Financeyear.split("-"); 
   ...
   let maxYear = Number(minYear) + 1;
	...
	function recPaybreakupDetails(T_FGLH_ID,T_FGLH_NAME,T_COMP_ID=""){
		...
		$('#ReceiptbreakupDetailForm').attr('action','<?=site_url()?>Trustfinance/RPledgerSummaryDetail');
