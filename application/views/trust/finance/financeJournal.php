    ...
    <div class="container-fluid container">
        <div class="row form-group">
            ...
            <div class="col-lg-3 col-md-10 col-sm-10 col-xs-8">
                <form id="frmCommitteeChange" action="<?=site_url()?>Trustfinance/Journal" method="post">
            ...
            <div class="col-lg-2 col-md-2 col-sm-2 col-xs-4" style = "padding-top:10px;">
                <a style="text-decoration:none;cursor:pointer;pull-right;" href="<?=site_url()?>Trustfinance/Journal" title="Refresh"><img style="width:24px; height:24px" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
            </div>
        </div>
        <form id="frmReceipt" action="<?=site_url()?>Trustfinance/addReceiptTrans" method="post" enctype="multipart/form-data"
                            ...
                            <select id="ledger_1" class="form-control ledger" name="ledger" style="width:100%" onChange="getLedgerChange(id)">
                                ...
                                <?php   if(!empty($ledger)) {
                                    foreach($ledger as $row1) { ?> 
                                        <option value="<?php echo $row1->T_FGLH_ID;?>|<?php echo $row1->BALANCE;?>|<?php echo $row1->T_FGLH_NAME;?>"><?php echo $row1->T_FGLH_NAME;?></option>
...
<script type="text/javascript">
    ...
    function validateJournal(done="false"){
        ...
        if($('#debitTot').val() == $('#creditTot').val()) {
            ...
            let url = "<?=site_url()?>Trustfinance/addJournalTrans";
            $('.ledger').each(function(i, ele) {
                pos = ele.id.split('_')[1];
            ...
            //    added the trim() by adithya on 28-12-23
                    if (e1[0].trim() === "success"){
                        ...
                        {'type': JSON.stringify(type),
                            ...
                            'todayDate': todayDate, 
                            'naration': naration,
                            'selCommittee':committeeSelectedId,
                            'committeeSelectedName':committeeSelectedName,
                            'toLedgerName':toLedgerName}, 
                            function(data) {
