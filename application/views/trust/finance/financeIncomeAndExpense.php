...
<div class="row form-group">							
	<div class="col-lg-4 col-md-10 col-sm-10 col-xs-8" style = "padding-right:0px;padding-top:10px;">
	 ...
	 <div class="col-lg-3 col-md-10 col-sm-10 col-xs-8">
        <form id="frmCommitteeChange" action="<?=site_url()?>Trustfinance/displayIncomeAndExpenditure" method="post">
...
<form id="frmOpenIncomeExpence" action="<?=site_url()?>Trustfinance/displayIncomeAndExpenditure" method="post" >
...
<div class="col-lg-12 col-md-12 col-sm-8 col-xs-8 pull-right text-right" style = "padding-right:0px;padding-bottom:10px;padding-top:10px; margin-top:-4em">
	<a style="text-decoration:none;cursor:pointer;margin-right: -30px" href="<?=site_url()?>Trustfinance/displayIncomeAndExpenditure" title="Refresh"><img style="width:24px; height:24px" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
</div>
							...
							if($row->T_LEVELS =='LG'){ ?>
								...
								<?php foreach($Cash as $row2) {
									if($row->T_FGLH_ID == $row2->T_FGLH_ID && $row2->CASH>0) {?> 
										...
										<td><a class="breakup" style="text-decoration:none;cursor: pointer;color:#ca7933 !important;" onClick="incomeexpensebreakupDetails('<?=$row2->T_FGLH_ID; ?>','<?php echo str_replace("'","\'",$row2->T_FGLH_NAME);?>','<?=$row2->T_COMP_ID; ?>')">&emsp;&emsp;-><?php echo $row2->T_COMP_NAME; ?></td>
...
let frommin =  $('#fromIe').val();
 ...
 let minmaxYear =Financeyear.split("-");
   ...
   let maxYear = Number(minYear) + 1;
	...
	function incomeexpensebreakupDetails(T_FGLH_ID,FGLH_NAME,COMP_ID=""){
		...
		$('#finnancebreakupDetailForm').attr('action','<?=site_url()?>Trustfinance/IEledgerSummaryDetail');
