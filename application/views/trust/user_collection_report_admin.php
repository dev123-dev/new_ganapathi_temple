<div class="container" >
	...
	<div class="row form-group">
		<div class="col-lg-8 col-md-8 col-sm-8 col-xs-8">
			<span class="eventsFont2">User Event Collection Report: <span class="samFont"><?php echo $events[0]->TET_NAME; ?></span></span>
		</div>
		<form id="approveId" enctype="multipart/form-data" method="post" accept-charset="utf-8">
			...
			<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
				<a style="width:24px; height:24px;cursor:pointer" class="pull-right" onclick="GetRefresh('<?=site_url()?>TrustReport/user_collection_report_admin')" title="Refresh"><img title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<div class="row form-group">
		<form id="userFilters" enctype="multipart/form-data" method="post" accept-charset="utf-8">
			<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
				<select id="users" name="users" class="form-control" onchange="GetDataOnUser(this.value,'<?php echo site_url()?>TrustReport/get_data_on_filter/1')">
			...
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
			  <select id="modeOfPayment" name="modeOfPayment" class="form-control" onchange="GetDataOnFilter(this.value,'<?php echo site_url()?>TrustReport/get_data_on_filter/1')">
		...
		<div class="col-lg-2 col-md-2 col-sm-4 col-xs-12">
			<div class="form-inline">
				...
				foreach($TotalAmount as $result) { 
					$amount = (int)($amount) + (int)($result->TET_RECEIPT_PRICE);
				} ?>
...
<script>
	...
	function GetRefresh(urlFirst) {
		url = "<?php echo site_url(); ?>TrustReport/get_unset_session";
	...
	function approveSubmit() {
		...
		clearSessionValues();
		url = "<?php echo site_url(); ?>TrustReport/approve_Submit";
	...
	$(document).ready(function() {
	    ...
	    if(sessionStorage.getItem("SelectedReceiptID") !== null && sessionStorage.getItem("SelectedReceiptID") != "") {
	   	...
	   	$('#eventCollectionContent').html('<table class="table "><thead><tr><th style="border:1px solid #7d6363"><center></center></th><th style="border:1px solid #7d6363">Date</th><th style="border:1px solid #7d6363">Receipt No.</th><th style="border:1px solid #7d6363">Receipt Type</th><th style="border:1px solid #7d6363"><center>Receipt Name</center></th><th style="border:1px solid #7d6363"><center>Estimated Price</center></th><th style="border:1px solid #7d6363"><center>Description</center></th><th style="border:1px solid #7d6363"><center>Payment Mode</center></th><th style="border:1px solid #7d6363"><center>Quantity</center></th><th style="border:1px solid #7d6363"><center>Amount</center></th><th style="border:1px solid #7d6363"><center>Postage</center></th><th style="border:1px solid #7d6363"><center>Total</center></th><th style="border:1px solid #7d6363"><center>Payment Status</center></th><th style="border:1px solid #7d6363"><center>Payment Notes</center></th><th style="border:1px solid #7d6363"><center>Entered By</center></th></tr></thead><tbody id="tblEventUserCollection"></tbody></table></div>')
	    for(var i = 0; i < arrItems.length; i++) {
    		...
    		if(sessionStorage.getItem("SelectAll") !== null && sessionStorage.getItem("SelectAll") == "true") {
    			...
    			$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px"><center><input id="checkerId_'+i+'" name="checkerId_'+i+'" type="checkbox" onchange="GetUnselect('+arrItems[i]["TET_RECEIPT_ID"]+',this.checked)" checked class="sel" /></center></td><td class="recId" id="recid_'+i+'" style="display:none;">'+arrItems[i]["TET_RECEIPT_ID"]+'</td>');
    		} else {
	    		if(sessionStorage.getItem("SelectedReceiptID") !== null && sessionStorage.getItem("SelectedReceiptID") != "") {
	    			if(arrSelectedReceiptID.indexOf(parseInt(arrItems[i]["TET_RECEIPT_ID"])) > -1) {
						...
						$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px"><center><input id="checkerId_'+i+'" name="checkerId_'+i+'" type="checkbox" onchange="GetUnselect('+arrItems[i]["TET_RECEIPT_ID"]+',this.checked)" class="sel" /></center></td><td class="recId" id="recid_'+i+'" style="display:none;">'+arrItems[i]["TET_RECEIPT_ID"]+'</td>');
					}
				}else{
					$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px"><center><input id="checkerId_'+i+'" name="checkerId_'+i+'" type="checkbox" onchange="GetUnselect('+arrItems[i]["TET_RECEIPT_ID"]+',this.checked)" class="sel" /></center></td><td class="recId" id="recid_'+i+'" style="display:none;">'+arrItems[i]["TET_RECEIPT_ID"]+'</td>');
				}
			...
			$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["TET_RECEIPT_DATE"] + '</td><td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["TET_RECEIPT_NO"] + '</td><td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["TET_RECEIPT_CATEGORY_TYPE"]+'</td><td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["TET_RECEIPT_NAME"]+'</td>');

			if(arrItems[i]["TET_RECEIPT_CATEGORY_TYPE"] == 'Inkind') { 
			...
			if(arrItems[i]["TET_RECEIPT_PAYMENT_METHOD"] == "Cheque") {
				$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px"><a class="log mymodelcancel" style="color:#800000;" onclick="show_cheque(1,\''+arrItems[i]["CHEQUE_NO"]+'\',\''+ arrItems[i]["CHEQUE_DATE"]+'\',\''+arrItems[i]["BANK_NAME"]+'\',\''+arrItems[i]["BRANCH_NAME"]+'\',\''+arrItems[i]["TRANSACTION_ID"]+'\')">'+arrItems[i]["TET_RECEIPT_PAYMENT_METHOD"]+'</a></td>');
    		} else if (arrItems[i]["TET_RECEIPT_PAYMENT_METHOD"] == "Credit / Debit Card") {
    			$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px"><a class="log mymodelcancel" style="color:#800000;" onclick="show_cheque(2,\''+arrItems[i]["CHEQUE_NO"]+'\',\''+arrItems[i]["CHEQUE_DATE"]+'\',\''+arrItems[i]["BANK_NAME"]+'\',\''+arrItems[i]["BRANCH_NAME"]+'\',\''+arrItems[i]["TRANSACTION_ID"]+'\')">'+arrItems[i]["TET_RECEIPT_PAYMENT_METHOD"]+'</a></td>');
    		} else {
    			$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["TET_RECEIPT_PAYMENT_METHOD"]+'</td>');
    		}
    		if(arrItems[i]["TET_RECEIPT_CATEGORY_TYPE"] == 'Inkind') { 
    		...
    		if(arrItems[i]["ET_RECEIPT_CATEGORY_TYPE"] != 'Inkind') { 
    			$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["TET_RECEIPT_PRICE"]+'</td>');
    		} else {
    			$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px">-</td>');
    		}
    		$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px">'+arrItems[i]["POSTAGE_PRICE"]+'</td>');
    		$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px">'+(parseInt(arrItems[i]["TET_RECEIPT_PRICE"]) + parseInt(arrItems[i]["POSTAGE_PRICE"]))+'</td>');
    		$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px">'+ arrItems[i]["PAYMENT_STATUS"] +'</td>');
    		$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px">'+ arrItems[i]["TET_RECEIPT_PAYMENT_METHOD_NOTES"] +'</td>');
    		$('#tblEventUserCollection').append('<td style="border:1px solid #7d6363;padding:10px">'+ arrItems[i]["TET_RECEIPT_ISSUED_BY"] +'</td>');
