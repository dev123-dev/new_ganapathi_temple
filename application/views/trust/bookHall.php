...
<div class="container">
	...
	<div class="row form-group">
		...
		<div style="float:right;" class="col-lg-6 col-md-6 col-sm-12 col-xs-12 ">
			<a style="width:24px; height:24px" class="pull-right img-responsive" href="<?=site_url(); ?>Trust/bookHall" title="Reset">
...
<div style="clear:both;" class="container">
	<div class="row form-group">
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<label class="radio-inline"><input type="radio" onclick="changeSubmitButtonText(1)" value="noPayment" name="pymtType" id="np" checked>No Payment</label>
			<label class="radio-inline"><input type="radio" onclick="changeSubmitButtonText(0);" value="advancePayment" name="pymtType" id="ap">Advance Payment</label>
		</div>
		...
		<div id="headsTrustDisplay" style="display:none;" class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="table-responsive">
				<table id="trustTable" class="table table-bordered">
					<thead>
						<tr>
							<th style="text-align:center;" colspan=5>Trust Receipt</th>
						</tr>
						<tr>
							<th style="text-align:left;">Name</th>
							<th style="text-align:left;">Amount</th>
							<th style="text-align:left;">Mode</th>
							<th style="text-align:left;">Pay Notes</th>
		...
		<div id="headsTempleDisplay" style="display:none;" class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="table-responsive">
				<table id="templeTable" class="table table-bordered">
					<thead>
						<tr>
						    <th style="text-align:center;" colspan=5>Temple Receipt</th>
							</tr>
						<tr>
							<th style="text-align:left;">Name</th>
							<th style="text-align:left;">Amount</th>
			...
			<div class="modal-body" style="overflow-y: auto;max-height: 80vmin;">
				...
				<div style="margin-bottom:10px;clear:both;">
					<div class="form-inline">
						...
						<input type="text" class="form-control form_contct2" id="amtModal" min=1 placeholder="" name="advAmt" onkeyup="checkPriceVal(event)">
					</div>
				</div>
				...
				<div style="clear:both;" class="form-inline">
					...
					<!-- temple -->
					<div style="padding-top: 15px; display:none;margin-left: -14px;" id="showDebitCredit_temple">
                    ...
                    <!--trust  -->
					<div style="padding-top: 15px; display:none;margin-left: -14px;" id="showDebitCredit_trust">
						<div class="control-group col-md-6 col-lg-6 col-sm-12 col-xs-12">
							<div class="form-inline" style="margin-bottom: 1em;" id="DCtobankDiv">
								<label for="bank">To Bank <span style="color:#800000;">*</span></label>&nbsp;&nbsp;
								<select id="trust_DCtobank" name="DCtobank" class="form-control">
								<option value="0">Select Bank</option>
								<?php foreach($terminal_trust as $result) { ?>
							...
							<input type="text" class="form-control form_contct2" id="trust_transactionId" placeholder="" name="transactionId">
						</div>
					</div>
					
					<!-- TEMPLE -->
					<div style="padding-top: 15px; display:none;margin-left: -14px; width: 70%;" id="showDirectCredit_temple">
					...
					<!-- TRUST -->
					<div style="padding-top: 15px; display:none;margin-left: -14px; width: 70%;" id="showDirectCredit_trust">
						<div class="form-group col-xs-10" id="tobankDiv">
							<label for="bank">To Bank <span style="color:#800000;">*</span></label>&nbsp;&nbsp;
							<select id="trust_tobank" name="tobank" class="form-control">
							<option value="0">Select Bank</option>
							<?php foreach($bank_trust as $result) { ?>
								<option value="<?=$result->T_FGLH_ID; ?>">
									<?=$result->T_FGLH_NAME; ?>
								</option>
								<?php } ?>
...
$('#modeOfPayment').on('change', function () {  
	                                                            //made the below change by adithya incase trust need separate bank and terminal
	if (this.value == "Cheque") {
		$('#showChequeList').fadeIn("slow");
		   $('#showDebitCredit_temple').fadeOut("slow");                                                               
		   $('#showDirectCredit_temple').fadeOut("slow");                                                
		   $('#showDebitCredit_trust').fadeIn("slow");                                                
		   $('#showDirectCredit_trust').fadeOut("slow");                                                
		...
		if($('#type').val()== "trust"){                                                
		   ...
		   $('#showChequeList').fadeOut("slow");
		   $('#showDebitCredit_temple').fadeIn("slow");
		   $('#showDirectCredit_temple').fadeOut("slow");
		   $('#showDebitCredit_trust').fadeOut("slow");
		   $('#showDirectCredit_trust').fadeOut("slow");
		}
		...
		if($('#type').val()== "trust") {
		   ...
		   $('#showDirectCredit_temple').fadeOut("slow");
		   $('#showDebitCredit_trust').fadeOut("slow");
		   $('#showDirectCredit_trust').fadeIn("slow");
		}
		else {
		   $('#showChequeList').fadeOut("slow");
		   $('#showDebitCredit_temple').fadeOut("slow");
		   $('#showDirectCredit_temple').fadeIn("slow");
		   $('#showDebitCredit_trust').fadeOut("slow");
		   $('#showDirectCredit_trust').fadeOut("slow");
		}
...
$('#submit').on('click', function () {
	...
	if (count != 0) {
		alert("Information", "Please fill required fields 123", "OK");
		return false;
	}
	...
	let url = "<?=site_url()?>Trust/hallBookingSave"   //Trust/hallBookingSave
	...
	$.post(url, {
		...
		if(!e)
			location.href = "<?=site_url(); ?>/Trust/printHallBooking";  ///Trust/printHallBooking
	});
});
...
function validateSubmit() {
	...
	if($('#advAmt').val() == "" && pymtType == "payment") {
	...
	$('.previewModalBody').append('<div class="table-responsive"><table class="table table-bordered"><thead><tr><th>Hall Name</th><th>Function Type</th><th>Date</th><th>From</th><th>To</th></tr></thead><tbody id="eventUpdate2"></tbody></table></div>')

	for (i = 0; i < tableContent.hallComboName.length; ++i) {
		$('#eventUpdate2').append("<tr>");
		$('#eventUpdate2').append("<td>" + tableContent['hallComboName'][i].innerHTML + "</td>");
...
function addRow() {
	...
	if(timepickerFrom == "" || timepickerTo == "") {
		...
		alert("Information", "Please fill required fields 789", "OK");
		return;
	}
	...
	$("#multiDate").datepicker({
		...
		onSelect: function (selectedDate) {
			try{
				...
				selBookTime = "";
				if(highlight != "noDates" && highlight != "" && typeof highlight !== 'undefined' && highlight!='invalid') {
					let fromDate = "";
					let toDate = "";
					let msg = "";
...
function addTrustHeads() {
	...
	let fglhBankTrust = "";
	if( modeOfPayment == "Direct Credit"){
		fglhBankTrust = $('#trust_tobank').val();

	}else if(modeOfPayment == "Credit / Debit Card"){
		fglhBankTrust = $('#trust_DCtobank').val();
	}
	let branch = $('#branch').val();
	let transactionId = $('#trust_transactionId').val();
	...
	if (modeOfPayment == "Cheque") {
		...
		$('#addHeadsTrust').append('<tr class="' 
		+ si + 'si1Trust si1Trust"><td style="display:none;" class="siTrust">' 
		+ si + '</td><td style="display:none;" class="transactionIdTrust">' 
		+ transactionId + '</td><td style="display:none;" class="branchTrust">' 
		+ branch + '</td><td style="display:none;" class="modeOfPaymentTrust">' 
		+ modeOfPayment + '</td><td style="display:none;" class="chequeNoTrust">' 
		+ chequeNo + '</td><td style="display:none;" class="chequeDateTrust">' 
		+ chequeDate + '</td><td style="display:none;" class="bankTrust">' 
		+ bank + '</td><td style="display:none;" class="idTrustHeads">' 
		+ idHeads + '</td>><td style="display:none;" class="typeTrustHeads">' 
		+ typeHeads + '</td><td class="nameTrustHeads">' 
		+ nameHeads + '</td><td class="amtTrust">' 
		+ amt + '</td><td class="modeOfPaymentTrust1">' 
		+ modeOfPayment + '</td><td class="pymtNotesTrust">'
		+pymtNotes+'</td><td style="display:none;"class="fglhBankTrust">'+fglhBankTrust+'</td><td class="link1Trust"><a style="cursor:pointer;" onClick="updateTrustTable(' 
		+ si + ');"><img style="width:24px; height:24px;" title="Delete" src="<?=site_url(); ?>images/delete1.svg"></a></td></tr>');
	}
...
function addHeads() {
	...
	let transactionId = $('#transactionId').val();
	let trust_transactionId = $('#trust_transactionId').val();
	let chequeNo = "";
	let bank = "";
	let chequeDate = "";
	let branch = "";
	let toBank = $('#tobank option:selected').val();//abhi
	let trust_toBank = $('#trust_tobank option:selected').val();
	let DCtoBank = $('#DCtobank option:selected').val();
	let trust_DCtobank = $('#trust_DCtobank option:selected').val()
	...
	if (modeOfPayment == "Cheque") {
		...
		if (branch) {
			...
			if (typeHeads== "temple") {						//laz new
				...
				if (trust_DCtobank != 0) {
					$('#trust_DCtobank').css('border-color', "#800000");
				} else {
					$('#trust_DCtobank').css('border-color', "#FF0000");
					++count;
				}
			}
			if(typeHeads == "temple") {
			...
			if(trust_transactionId){
				$('#trust_transactionId').css('border-color', "#800000");
			}
			else {
				$('#trust_transactionId').css('border-color', "#FF0000");
				++count;
			}	
			}
		...
		if(trust_tobank != 0){
			$('#trust_tobank').css('border-color', "#800000");
			} else {
				$('#trust_tobank').css('border-color', "#FF0000");
				++count;
			}			
		$('#chequeNo').css('border-color', "#800000");
		$('#branch').css('border-color', "#800000");
