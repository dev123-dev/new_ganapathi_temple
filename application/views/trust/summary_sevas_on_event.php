...
<div style="clear:both;" class="container">
	...
	<div class="row form-group">							
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">	
			...
			<a class="pull-right" style="margin-left:5px;" href="<?=site_url()?>TrustReport/summary_sevas_on_event" title="Refresh"><img style="width:24px; height:24px;" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
	</div>	
				...
				<table class="table table-bordered table-hover">
					...
					<tbody>
						<?php $Count = 0; foreach($report_details as $result) { 
							...
							$Count += $result['QTY'];						?>
							<tr class="row1">
								<td><a style="color:#800000;" onClick = "get_display_deity_details('<?php echo $result['TET_SO_SEVA_ID']; ?>','<?php echo $result['TET_SO_SEVA_NAME']; ?>')"><?php echo $result['TET_SO_SEVA_NAME']; ?></a></td>
...
<script>
	...
	$('#EveryRadio').on('click', function() {
		...
		document.getElementById('radioOptField').value = "multiDate";
		url = "<?php echo site_url(); ?>TrustReport/summary_sevas_on_event";
	...
	//ON CHANGE OF DATEFIELD
	 function get_datefield_change(date) {
		...
		url = "<?php echo site_url(); ?>TrustReport/get_filter_change_event";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	} 
	...
	function GetSendField() {
		...
		url = "<?php echo site_url(); ?>TrustReport/event_sevas_summary_report_excel";
	...
	function generatePDF() {
		...
		url = "<?php echo site_url(); ?>generatePDF/create_trusteventSevaSummaryReceiptpdf";
		$("#report").attr("action",url)
		$("#report").submit();
	}
	...
	//CREATE PRINT
	function print() {
		let url = "<?php echo site_url(); ?>generatePDF/create_trusteventSevaSummaryReceiptSession";
		...
		$.post(url,{'dateField':$('#todayDate').val(),'radioOpt':radioOpt,'fromDates':fromDates,'toDates':toDates}, function(data) {
			let url2 = "<?php echo site_url(); ?>generatePDF/create_trusteventSevaSummaryReceiptPrint";
			if(data == 1) {
				downloadClicked = 0;
				var win = window.open(
				  url2,
