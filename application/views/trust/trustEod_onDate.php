...
<div class="container">
	...
	<div class="row form-group">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
							...
							<?php 
								...
								<?php if($result->AUTHORISED_STATUS != 0) { ?>
									<?php if($selectedDate != date('d-m-Y') || strtotime(date('H:i:s')) >= strtotime($timeSettings['TIME_FROM']) && strtotime(date('H:i:s')) <= strtotime($timeSettings['TIME_TO'])) { ?>
										<td><a style='cursor:pointer;color:#800000;text-decoration:none;' onClick="eodUserCollection('<?php echo $selectedDate; ?>','<?php echo $result->ENTERED_BY_NAME; ?>','<?php echo $result->ENTERED_BY; ?>')"><?php echo $result->ENTERED_BY_NAME; ?></a></td>
									<?php } else { ?>
								...
								<?php } else { ?>
									<td>Yes</td>
								<?php } ?>
								<td><?php if($_SESSION['userId'] != "1" && $_SESSION['userId'] != "2" && $_SESSION['userId'] != "6") { ?><center><button onClick="eodUserCollectionViewBtn('<?php echo $selectedDate; ?>','<?php echo $result->ENTERED_BY_NAME; ?>','<?php echo $result->ENTERED_BY; ?>')" style="width: 72px;" type="button" class="btn btn-default btn-sm">View</Button></center><?php } ?></td>
							</tr>
			...
			<?php if($receiptType == "generate" && $TotalAuthorised == 0) { ?>
				<button type="button" class="btn btn-default btn-lg pull-right" onClick='alertTEOD("Confirm","Are you sure, you want to Generate EOD","Yes",false,"action")'>Confirm E.O.D.</button>
			<?php } else if($receiptType == "generate" && $TotalAuthorised != 0) { ?>
				<button disabled type="button" class="btn btn-default btn-lg pull-right" style="background: #b8b8bd;color:black;" onClick='alertTEOD("Confirm","Are you sure, you want to Generate EOD","Yes",false,"action")'>Confirm E.O.D.</button>
			<?php } else { ?>
	...
	<form id="eodForm" method="post" action="<?=site_url()?>TrustEOD/trustEod_onDate/">
...
<script>
	function eodOnDate(date) {
		$('#date').val(date);
		$('#eodOnDate').attr('action',"<?=site_url()?>TrustEOD/trustEod_collection/");
		$('#eodOnDate').submit();
	}
	function eodUserCollection(date,username,uid) {
		let url = "<?=site_url()?>TrustEOD/checkPreviousPendingDate/";
		...
		$.post(url, {
			...
			if(e == "success") {
				...
				$('#userId').val(uid);
				$('#eodUser').attr('action',"<?=site_url()?>TrustEOD/trustEod_usercollection/");
				$('#eodUser').submit();
			} else {
	...
	function eodUserCollectionViewBtn(date,username,uid) {
			...
			$('#eodUser').attr('action',"<?=site_url()?>TrustEOD/trustEodUsercollection/");
			$('#eodUser').submit();
	...
	function print(date) {
		console.log(date);
		let url = "<?php echo site_url(); ?>generatePDF/create_trustEodReceiptSession";
		$.post(url,{'date':date}, function(data) {
			let url2 = "<?php echo site_url(); ?>generatePDF/create_trustEodReceiptPrint";
			if(data == 1) {
				downloadClicked = 0;
				var win = window.open(
				  url2,
