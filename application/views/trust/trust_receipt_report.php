<div class="container">
	...
	<div class="row form-group">
		<div class="col-lg-10 col-md-10 col-sm-10 col-xs-8">
			<span class="eventsFont2">Trust Receipt Report</span>
		</div>
		<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">
			<a style="width:24px; height:24px" class="pull-right img-responsive" href="<?=site_url()?>/TrustReport/trust_receipt_report" title="Reset"><img title="reset" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
	</div>
	...
	<div class="row form-group">
		<form id="tddate" enctype="multipart/form-data" method="post" accept-charset="utf-8">
			...
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-9" style="margin-bottom:1em;display:none;">
			  <select id="financialHead" name="financialHead" class="form-control" onchange="get_deity_change(this.value)">
				<option value="All Financial Head">All Financial Head</option>
				<?php if(isset($financialHeadId)) { ?>
					<?php foreach($financialHead as $result) { ?>
						<?php if($financialHeadId == $result->FH_ID) { ?>
							<option value="<?php echo $result->FH_ID; ?>" selected><?php echo $result->FH_NAME; ?></option>
						<?php } else { ?>
			...
			<div class="col-lg-2 col-md-2 col-sm-6 col-xs-12">
				<div class="form-inline">
					...
					foreach($TotalAmount as $result) { 
						$amount = (int)($amount) + (int)($result->FH_AMOUNT);
					} ?>
		...
		<form id="report" enctype="multipart/form-data" method="post" accept-charset="utf-8">
			...
			<input type="hidden" name="financialHeadId" id="financialHeadId">
...
<div class="container">
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="table-responsive">
				<table class="table table-bordered table-hover">
					...
					<tbody>
						<?php foreach($trust_receipt_report as $result) { ?>
							<tr class="row1">
								<?php if($result->PAYMENT_STATUS == "Cancelled") { ?>
									<td style="text-decoration:line-through;"> <?php echo $result->TR_NO; ?></td>
								<?php } else { ?>
									<td> <?php echo $result->TR_NO; ?></td>
								<?php } ?>
								<?php if($result->RECEIPT_CATEGORY_ID == 1) { ?>
									<td>Inkind</td>
								<?php } else { ?>
									<td>Trust Receipt</td>
								<?php } ?>
								...
								<td style="width:20%;word-break: break-all;"><?php echo $result->TR_PAYMENT_METHOD_NOTES; ?></td>
	...
	<div class= "row">
		...
		<!--Total Sevas TextField -->
			<label class="pull-right" for="sevaAmount" style="font-size:18px;margin-right:15px;margin-top: -1em;">Total Receipt: <span id="totalSeva"><?php echo $Count; ?></span></label>  				
	</div>
</div>
<!-- Cheque Modal2 -->
...
<script>	
	...
	function GetSendField() {
		...
		document.getElementById('financialHeadId').value = $('#financialHead').val();
		url = "<?php echo site_url(); ?>TrustReport/trust_receipt_report_excel";
	...
	function generatePDF() {
		...
		document.getElementById('dateField').value = $('#todayDate').val();
		document.getElementById('financialHeadId').value = $('#financialHead').val();
		url = "<?php echo site_url(); ?>generatePDF/create_trustReceiptpdf";
		$("#report").attr("action",url)
		$("#report").submit();
	}
	
	function print() {		
		let url = "<?php echo site_url(); ?>generatePDF/create_trustReceiptSession";
		radioOpt = document.getElementById('radioOptField').value;
		allDates = document.getElementById('allDates').value;
		$.post(url,{'payMode':$('#modeOfPayment').val(),'dateField':$('#todayDate').val(),'financialHeadId':$('#deity').val(),'radioOpt':radioOpt, 'allDates':allDates}, function(data) {
			let url2 = "<?php echo site_url(); ?>generatePDF/create_trustReceiptPrint";
			if(data == 1) {
					downloadClicked = 0;
					var win = window.open(
					  url2,
	...
	function get_datefield_change(date) {
		...
		document.getElementById('financialHeadId').value = $('#financialHead').val();
		url = "<?php echo site_url(); ?>TrustReport/trust_report_on_change_date";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	}
	
	//ON CHANGE OF DEITY
	function get_deity_change(financialHeadId) {
		var radio = $('#radioOpt').val();
		let count = 0;
		if(radio == "date"){
			if(!$('#todayDate').val()) {
		...
		document.getElementById('paymentMethod').value = $('#modeOfPayment').val();
		document.getElementById('financialHeadId').value = financialHeadId;
		url = "<?php echo site_url(); ?>TrustReport/trust_report_on_change_date";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	}
	...
	$('#EveryRadio').on('click', function() {
		...
		url = "<?php echo site_url(); ?>TrustReport/trust_receipt_report";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	});
