...
<div class="container">
	...
	<div class="row form-group">
		...
		<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">
			<a style="width:24px; height:24px" class="pull-right img-responsive" href="<?=site_url()?>TrustEOD/eod_admin" title="Refresh"><img title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
	</div>
</div>
...
<script>
	...
	$('#submit').on('click', function () {
		...
		let url = "<?=site_url()?>TrustReceipt/save_function_details"
		$.post(url, {'postVal':postValue}, function(data) {
			if(data == "success") {
				...
				$('#eodOnDate').attr('action',"<?=site_url()?>TrustEOD/trustEod_onDate/");
				$('#eodOnDate').submit();
			}
		});
	});
	...
	function eodOnDate(date, receiptType) {
		...
		if(receiptType == 'generate') {
			...
			let url = "<?=site_url()?>TrustReceipt/get_function_details";
