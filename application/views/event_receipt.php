	...
	<div class="row form-group">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			...
			<div class="col-lg-6  col-md-6 col-sm-7 col-xs-10">
				<span class="eventsFont2 samFont1"><?=$event['ET_NAME']; ?></span>
			</div>
			<div class="col-lg-2 col-md-2 col-sm-1 col-xs-2">
				<a style="width:24px; height:24px" class="pull-right img-responsive" href="<?=site_url()?>Events/event_receipt" title="Reset"><img title="Reset" src="<?=site_url();?>images/refresh.svg"/></a>
			</div>
		</div>
	</div>
...
<div class="row">
			  ...
			  <select id="sevaCombo" class="form-control">
			   <option value="" selected="selected">Select Seva Name</option>
				<?php foreach($eventSeva as $result) { ?>
					<option value="<?=$result['ET_SEVA_PRICE'].'|' . $result['ET_SEVA_QUANTITY_CHECKER'] . "|".$result['IS_SEVA'] ."|".$result['ET_SEVA_ID']."|".$result['RESTRICT_DATE']; ?>">
						<?=$result['ET_SEVA_NAME']. " = &#8377; " .$result['ET_SEVA_PRICE']; ?>
					</option>
				<?php } ?>
			  </select>
			</div>
	...
	$('#submit').on('click', function() {
		...
		let etName = "<?=@$event['ET_NAME']; ?>";
		let etId = "<?=@$event['ET_ID']; ?>";
		...
		if(count == 0) {
			...
			$.post(url,{'transactionId':transactionId,'etName':etName,'etId':etId,'chequeNo':chequeNo,'branch':branch,'bank':bank,'chequeDate':chequeDate,'modeOfPayment':modeOfPayment,'hiddenEventId':JSON.stringify(hiddenEventId),'hiddenIsSeva':JSON.stringify(hiddenIsSeva),'tableSevaCombo':JSON.stringify(tableSevaCombo),'tableQty':JSON.stringify(tableQty), 'tableDate':JSON.stringify(tableDate), 'tablePrice':JSON.stringify(tablePrice),'tableAmt':JSON.stringify(tableAmt), 'totalAmt':total,'name':name,'number':number,'rashi':rashi,'nakshatra':nakshatra,'paymentNotes':paymentNotes,'postage': postage, 'postageAmt': postageAmt, 'addressLine1': addressLine1, 'addressLine2': addressLine2, 'city': city, 'country': country, 'pincode': pincode,'address':address,'fglhBank': fglhBank }, function(e) {
				
				e1 = e.split("|")
				if(e1[0] == "success")
					location.href = "<?=site_url();?>Events/printSevaReceipt";
				else if(e1[0] == "stock") {
	...
	//check for which form is displayed ends
		...
		$('#sevaCombo').on('change', function() {
			...
			$('#multiDate').multiDatesPicker('resetDates');
			$('#multiDate1').datepicker('setDate', null);
			$.post("<?=site_url()?>Events/getSevaLimit", {'idName':eventSeva[3],'stock':eventSeva[2]}, function(e) {
				limit = JSON.parse(e);
				console.log(limit)
				if(limit.length > 0) {
					if(eventSeva[1] == 1 && eventSeva[2] != 1) {
						...
						for(let i = 0; i < limit.length; ++i) {
								limitStock += Number(limit[i]['ET_SEVA_LIMIT']);
						}
						
						console.log(limitStock);
						
						$.post('<?=site_url()?>Events/getStock', {'id':eventSeva[3],'stock':eventSeva[2]}, function(d) {
							
							var totalStock = JSON.parse(d);
							console.log(totalStock)
							var totalStock1 = 0;
							for(let i = 0; i < totalStock.length; ++i) {
									totalStock1 += Number(totalStock[i]['ET_SO_QUANTITY']);
							}
	...
	function checkConfirmTime(date) {
		$.ajax({
            type: "POST",
            url: "<?php echo base_url();?>Events/check_event_eod_confirm_time",
	...
	$("#multiDate1").datepicker({
		...
		onSelect: function (selectedDate) {
			...
			$('#multiDate').blur();
			$('#multiDate').css('border-color', "#000000");
			$.post('<?=site_url()?>Events/getSevaOffered',{'id':eventSeva[3], 'sevadate':selectedDate}, (e)=>{
				if(e.length > 0) {
