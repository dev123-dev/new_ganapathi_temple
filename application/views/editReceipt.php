	...
	<!--for printing --><div id="printScreen" style="display:none">
		<page style="margin-top:25px;margin-left:75%;width:115%;margin-right:75%;">
			...
			<div style="margin-top:45px;">
				...
				<span class="eventsFont2" style="font-size:11px;letter-spacing:1px;">Receipt Date&nbsp;: <?=$eventCounter[0]["ET_RECEIPT_DATE"];?></span>
				<span style="font-size:11px;letter-spacing:1px;" class="eventsFont2">Receipt No&nbsp;&nbsp;&nbsp;&thinsp;: <?=$eventCounter[0]['ET_RECEIPT_NO'] ?></span>
				<div style="margin-bottom:5px;"></div>
				<span style="font-size:11px;letter-spacing:1px;"><strong>Name&emsp;&ensp;&ensp;&nbsp;&thinsp;: </strong><?=$eventCounter[0]['ET_RECEIPT_NAME'] ?></span>
				<?php if($eventCounter[0]['ET_RECEIPT_PHONE'] != "") { ?>
					<span style="font-size:11px;letter-spacing:1px;"><strong>Number&ensp;&ensp;&ensp;: </strong><?=$eventCounter[0]['ET_RECEIPT_PHONE'] ?></span>
				<?php } ?>
				<?php if($eventCounter[0]['ET_RECEIPT_RASHI'] != "") { ?>
					<span style="clear:both;letter-spacing:1px;font-size:11px;"><strong>Rashi&ensp;&ensp;&ensp;&nbsp;&nbsp;&thinsp;&thinsp;: </strong><?=$eventCounter[0]['ET_RECEIPT_RASHI'] ?></span><br/>
				<?php } ?>
				<?php if($eventCounter[0]['ET_RECEIPT_NAKSHATRA'] != "") { ?>
					<span style="letter-spacing:1px;font-size:11px;"><strong>Nakshatra&nbsp;: </strong><?=$eventCounter[0]['ET_RECEIPT_NAKSHATRA'] ?></span>
				<?php } ?>
				<br/>
				<br/><br/>
				...
				<br/>
				<span style="font-size:11px;float:right;letter-spacing:1px;" class="eventsFont2">Total Amount&nbsp;: <?=$subTotal ?><?=AmountInWords($subTotal);?></span><br/>
				<span style="font-size:11px;letter-spacing:1px;"><strong>Mode Of Payment&nbsp;: </strong><?=$eventCounter[0]['ET_RECEIPT_PAYMENT_METHOD']; ?></span><br/>
	...
	<div class="container">
		<form class="form-group">
			  <div class="form-group">
				<center><label class="eventsFont2 samFont1"><?=$eventCounter[0]["RECEIPT_ET_NAME"]; ?></label></center>
			  ...
			  <div class="form-group">
				...
				<span style="float:right" class="eventsFont2">Receipt Number: <?=$eventCounter[0]['ET_RECEIPT_NO'] ?></span>
			  </div>
			  
			  <div class="form-group">
				<span style="font-size:18px;"><strong>Name: </strong><?=$eventCounter[0]['ET_RECEIPT_NAME'] ?></span>
				<?php if($eventCounter[0]['ET_RECEIPT_RASHI'] != "") { ?>
					<span style="clear:both;float:right;font-size:18px;"><strong>Rashi: </strong><?=$eventCounter[0]['ET_RECEIPT_RASHI'] ?></span>
				<?php } ?>
			  </div>
			   ...
			   <?php if($eventCounter[0]['ET_RECEIPT_PHONE'] != "") { ?>
				...
				<?php } ?>
				<?php if($eventCounter[0]['ET_RECEIPT_NAKSHATRA'] != "") { ?>
					<span style="float: right;font-size:18px;"><strong>Nakshatra: </strong><?=$eventCounter[0]['ET_RECEIPT_NAKSHATRA'] ?></span>
				<?php } ?>
			  </div>
			  ...
			  <div class="table-responsive">
				<table id="eventSeva" class="table table-bordered">
					...
					foreach($eventCounter as $result) {
						...
						$total = ($result["ET_SO_PRICE"] * $qty);
						$subTotal += $total;
						$ET_SO_ID = $result['ET_SO_ID'];
						$ET_SO_SEVA_ID = $result["ET_SO_SEVA_ID"];
						$ET_SO_IS_SEVA = $result["ET_SO_IS_SEVA"];
						echo "<tr><td>".$i++."</td>";
						echo "<td>". $result["ET_SO_SEVA_NAME"]."</td>";
						echo "<td>". $qty."</td>";
						echo "<td>". $result["ET_SO_DATE"]."</td>";
						echo "<td>". $result["ET_SO_PRICE"]."</td>";
						echo "<td>". $total ."</td>";
						if($result["ET_UPDATED_SO_DATE"] == "" && $result["ET_SO_IS_SEVA"] == "1" && strtotime($result["ET_SO_DATE"]) >= strtotime(date('d-m-Y')))
							echo "<td style='cursor:pointer;border-color: transparent;'><a title='Edit' onClick='edit($ET_SO_ID, $ET_SO_SEVA_ID, $ET_SO_IS_SEVA)';><span style='color:#800000' class='glyphicon glyphicon-pencil'></span></a></td></tr>";
					}
					?>
					</tbody>
				</table>
			   ...
			   <div class="form-group">
				...
				<?php } ?>
					<span style="font-size:18px;float:right"><strong>Issued By: </strong><?=$eventCounter[0]['ET_RECEIPT_ISSUED_BY'] ?></span>
			  </div>
		</form>
...
<script>
	...
	var fromDate = "<?=$activeDate[0]->ET_FROM_DATE_TIME; ?>"; 
	var toDate = "<?=$activeDate[0]->ET_TO_DATE_TIME; ?>"; 
	...
	function edit(id, sevaId, isSeva) {
		...
		$.post("<?=site_url()?>Events/getSevaLimit", {'idName':sevaId1, 'fromEditReceiptPrint':"1"}, function(e) {
				...
				if(limit.length > 0) {
					...
					for(let i = 0; i < limit.length; ++i) {
						availableDates[i] = limit[i]['ET_SEVA_DATE'].replace(/(^|-)0+/g, "$1");
					}
					$('#multiDate1').focus();
				}else {
					$('.multiDate').slideDown();
	...
	$('#update').on('click', function() {
		...
		$.post('<?=site_url()?>Events/getSevaOffered',{'id':sevaId1, 'sevadate':multiDate}, (e)=>{
			if(e.length > 0) {
				var currentSeva = JSON.parse(e);
				currentSevaLength = currentSeva.length;
				if(limit.length > 0) {
					for(let i = 0; i < limit.length; ++i) {
						if(limit[i]['ET_SEVA_DATE'] == multiDate) {
							currentSevaLength = (limit[i]['ET_SEVA_LIMIT'] - currentSevaLength);
							break;
						}
					}
					if(Number(currentSevaLength) > 0) {
						$('#sevaAvailable').html(currentSevaLength);
						$.post("<?=site_url()?>Events/updateReceipt",{'multiDatehidden':multiDate,'updateId':id1}, function(e) {
							if(e = "success") {
								$.confirm({
									title: "Information",
	...
	$("#multiDate1").datepicker({
		...
		onSelect: function (selectedDate) {
			$.post('<?=site_url()?>Events/getSevaOffered',{'id':sevaId1, 'sevadate':selectedDate}, (e)=>{
				if(e.length > 0) {
					var currentSeva = JSON.parse(e);
					currentSevaLength = currentSeva.length;
					if(limit.length > 0) {
						...
						for(let i = 0; i < limit.length; ++i) {
							if(limit[i]['ET_SEVA_DATE'] == selectedDate) {
								currentSevaLength = (limit[i]['ET_SEVA_LIMIT'] - currentSevaLength);
								break;
							}
						}
