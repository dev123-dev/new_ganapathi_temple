<form id="formSub" action="<?php echo site_url(); ?>Receipt/receipt_deity_kanike_save" enctype="multipart/form-data" method="post" accept-charset="utf-8">
	<div class="container">
		...
		<div class="row form-group">
			<div class="col-lg-10 col-md-10 col-sm-10 col-xs-8">
				<span class="eventsFont2">Deity Kanike Receipt </span>
			</div>
			<br/>
			<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">
				<a style="width:24px; height:24px" class="pull-right img-responsive" href="<?=site_url()?>Receipt/receipt_deity_kanike" title="Reset"><img title="Reset" src="<?=site_url();?>images/refresh.svg"/></a>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col-md-6 text-left">
				...
				<div class="form-inline col-lg-12 col-md-12 col-sm-12 col-xs-8" id="kanikeDiv" style="margin-bottom: 1.5em;">
					<div class="form-group">
						<label for="kanikeFor">Kanike for </label>&nbsp;&nbsp;&nbsp;&nbsp;
						<select id="kanikeFor" name="kanikeFor" class="form-control" onChange="kanikeForChange();">
							<?php foreach($kanikeFor as $result) { ?>
								<option value="<?php echo $result->KS_ID ."|". $result->KANIKE_NAME ."|". $result->PRICE ?>"><?php echo $result->KANIKE_NAME ?></option>
							<?php } ?>
						</select>
					</div>
				</div>
	  ...
	  <div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title">Deity Kanike Preview</h4>
			</div>
...
<script>
	...
	function deityComboChange() {
		...
		bgNo = arrDeity[0];
		deityComboName = arrDeity[1];

		if(bgNo == 1){
			document.getElementById("kanikeDiv").style.display ="block";
			$('#kanikeFor').val("1|General Kanike|0");
		}
		else{
			document.getElementById("kanikeDiv").style.display ="none";
			$('#kanikeFor').val("1|General Kanike|0");
			// $('#kanikeFor').append('<option value="1|General Kanike" selected>General Kanike</option>');
	...
	function kanikeForChange() {
		let amt = ($('#kanikeFor').val()).split('|');
		if (amt[2]>0) {
			$('#amount').val(amt[2]);
		}
	}
