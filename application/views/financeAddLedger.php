               ...
               <select id="group" name="group" class="form-control" onchange="showRequiredOption();"style="height: 30px; width: 200px;">
                        ...
                        <?php   if(!empty($groups)) {
                            foreach($groups as $row1) { ?> 
                                <option value="<?php echo $row1->FGLH_ID;?>|<?php echo $row1->FGLH_NAME;?>|<?php echo $row1->TYPE_ID;?>"><?php echo $row1->FGLH_NAME;?></option>
                            <?php } } ?>
                </select>         
            </div>   
        </div>
...
<!-- new FD NAME and Bank name section End  -->
        ...
        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-6" id="opDate">              
            <div class="input-group input-group-sm col-lg-2" style="width: 120px;">
                ...
                <input autocomplete="" name= "todayDate3" id="todayDate3" type="text" value="<?=date('d-m-Y'); ?>" class="form-control fdstartDate"  onchange="GetDataOnDate(this.value)" placeholder="dd-mm-yyyy" readonly = "readonly" />
...
// adding the condition by adithya to add opening bal for only A & L
        ...
        if(count != 0) {
            ...
            if(flag!=0){
                ...
                if (!si)
                    ...
                    ++si;
                $('#addCommittee').append('<tr class="' + si + 
                                          ' si1"><td class="si">' + si + 
                                          '</td><td class="committeeId committeeIdCheck" id="' + ledId[0] + 
                                          '" style="display:none">' + ledId[0] + 
                                          '</td><td class="Committee">' + Committee + 
                                          '</td><td class="ComOpAmt">' + committeeOpAmt + 
                                          '</td><td class="link1"><a style="cursor:pointer;" onClick="updateTable1(' + si + 
                                          ');"><img style="width:24px; height:24px;" title="delete" src="<?=base_url()?>images/delete1.svg"></a></td></tr>');
                $('#committeeOpAmt').val("");
