					...
					<section class="body">
						<div class="row form-group"> 
							<div class="col-md-12 col-lg-12 col-sm-12 col-xs-12 no-pad">
								<h3 style="font-weight:600">Trust Cheque Reconciliation</h3>
							</div>
						</div>
						<form action="<?php echo site_url(); ?>admin_settings/Admin_Trust_setting/SearchtrustChequeRemmittance" id="chequeNumberSubmit" enctype="multipart/form-data" 	 method="post" accept-charset="utf-8">
							<div class="row form-group" style="margin-top:-0.8em;"> 
								...
								<a style="text-decoration:none;cursor:pointer;pull-right;" href="<?=site_url()?>admin_settings/Admin_Trust_setting/trustChequeRemmittance" title="Refresh"><img style="width:24px;height:24px;margin-top:10px;" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
								</div>
							</div>
						</form>
						<div class="body-inner no-padding table-responsive">
							<table class="table table-bordered table-hover">
								...
								<tbody>
									...
									<?php foreach($trustCheckRemmittance as $result) { ?>										
										<tr class="row1">
											<td><?php echo $result->T_VOUCHER_NO;?></td>
											<td><?php echo $result->T_FLT_DEPOSIT_PAYMENT_DATE;?></td>
											<td><?php echo $result->T_VOUCHER_TYPE; ?></td>
											<?php $receiptId = $result->T_RECEIPT_ID; ?>
											<?php $voucherNo = $result->T_VOUCHER_NO; ?>
											<?php $voucherType = $result->T_VOUCHER_TYPE; ?>
											<?php $name = str_replace("'","\'",$result->T_RECEIPT_FAVOURING_NAME); ?>
											<?php $amt = $result->AMT; ?>
											<?php $chequeNo = $result->T_CHEQUE_NO; ?>
											<?php $chequeDate = $result->T_CHEQUE_DATE; ?>
											<?php $bank = str_replace("'","\'",$result->T_BANK_NAME); ?>
											<?php $branch = str_replace("'","\'",$result->T_BRANCH_NAME); ?>
											<?php $DEPOSITED_BANK = $result->T_DEPOSITED_BANK; ?>
											
											<td><?php echo $result->T_RECEIPT_FAVOURING_NAME; ?></td>
											<td><?php echo $result->AMT; ?></td>
											<td><?php echo $result->T_CHEQUE_NO; ?></td>
											<td><?php echo $result->T_CHEQUE_DATE; ?></td>
											<td><?php echo $result->T_BANK_NAME; ?></td>
											<td><?php echo $result->T_BRANCH_NAME; ?></td>
											<td><?php echo $result->T_PAYMENT_STATUS; ?></td>
											<?php if(isset($_SESSION['Authorise'])) { ?>
												<td class="text-center" width="30%">

													<?php if($result->T_PAYMENT_STATUS == "Pending") { ?>
														<a style="border:none; outline: 0;" onClick="callModal('<?=$name; ?>','<?=$amt; ?>','<?=$chequeNo; ?>','<?=$chequeDate; ?>','<?php echo str_replace("'","\'",$result->T_BANK_NAME);?>','<?php echo str_replace("'","\'",$result->T_BRANCH_NAME);?>','<?=$receiptId; ?>','<?=$voucherNo; ?>','<?=$voucherType; ?>','<?php echo str_replace("'","\'",$result->T_DEPOSITED_BANK);?>');" title="Approve Cheque" ><img style="border:none; outline: 0;" width="24px" height="24px" src="<?php echo	base_url(); ?>images/check_icon.svg"></a>

													<?php } ?>
													<?php if($result->T_VOUCHER_TYPE != "Receipt") { ?>
														<a  style="margin-left: 5px; " style="border:none; outline:0;"  title="Cancel Cheque" onClick="callCancelModal('<?=$result->T_CHEQUE_NO; ?>','<?=$result->T_VOUCHER_NO;?>');"><img style="border:none; outline: 0; width:24px; height:24px" src="<?php echo base_url();?>images/trash.svg"></a>
													<?php } ?>
													<?php if($result->T_VOUCHER_TYPE == "Receipt") { ?>
													<a  style="margin-left: 5px; " style="border:none; outline:0;"  title="Replace Cheque" onClick="callReplaceModal('<?=$chequeNo; ?>','<?=$chequeDate; ?>','<?php echo str_replace("'","\'",$result->T_BANK_NAME);?>','<?php echo str_replace("'","\'",$result->T_BRANCH_NAME);?>','<?=$receiptId; ?>','<?=$voucherNo; ?>','<?php echo str_replace("'","\'",$result->T_DEPOSITED_BANK);?>');"><img style="border:none; outline: 0; width:24px; height:24px" src="<?php echo base_url();?>images/replace.svg"></a>
													<?php } ?>
												</td>
	...
	<div class="modal-dialog">
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 style="font-weight:600;" class="modal-title text-center">Trust Cheque Reconciliation</h4>
			</div>
...
<form id="submitForm" action="<?php echo site_url(); ?>admin_settings/Admin_Trust_setting/edit_trustChequeReconcilation/" id="chequeRemmittanceForm" class="form-group" role="form" enctype="multipart/form-data" method="post">
	...
	<input type="hidden" id="voucherT" name="voucherType" value="">
</form>
...
<form id="submitCancellationForm" action="<?php echo site_url(); ?>Trustfinance/cancelCheque" class="form-group" role="form" enctype="multipart/form-data" method="post">	
	...
	<input type="hidden" id="cancellationRedirectPath" name="cancellationRedirectPath" value="trustChequeRemmittance">

</form>

  <form id="submitReplaceForm" action="<?php echo site_url(); ?>Trustfinance/replaceChequeDetails" class="form-group" role="form" enctype="multipart/form-data" method="post">	
