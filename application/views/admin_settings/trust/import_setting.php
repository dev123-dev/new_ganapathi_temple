					...
					<section class="body">
						<div class="row form-group"> 
							...
							<form id="dateChange" enctype="multipart/form-data" method="post" accept-charset="utf-8">
								<div class="col-lg-2 col-md-2 col-sm-2 col-xs-8">
									<div class="input-group input-group-sm form-group"> 
										<input id="todayDate" name="todayDate" type="text" value="<?php echo $date; ?>" class="form-control todayDate2" placeholder="dd-mm-yyyy" onchange="GetDataOnDate(this.value,'<?php echo site_url()?>admin_settings/Admin_Trust_setting/get_data_on_filter')">
								...
								<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
									<select id="users" name="users" class="form-control" onchange="GetDataOnUser(this.value,'<?php echo site_url()?>admin_settings/Admin_Trust_setting/get_data_on_filter')">
										<option selected value="All Users">All Users</option>
										<?php foreach($usersCombo as $result) { ?>
											<?php if(isset($user)) { ?>
												<?php if($user == $result->TET_RECEIPT_ISSUED_BY_ID) { ?>
													<option selected value="<?php echo $result->TET_RECEIPT_ISSUED_BY_ID; ?>"><?php echo $result->TET_RECEIPT_ISSUED_BY; ?></option>
												<?php } else { ?>
													<option value="<?php echo $result->TET_RECEIPT_ISSUED_BY_ID; ?>"><?php echo $result->TET_RECEIPT_ISSUED_BY; ?></option>
												<?php } ?>
											<?php } else { ?>
												<option value="<?php echo $result->TET_RECEIPT_ISSUED_BY_ID; ?>"><?php echo $result->TET_RECEIPT_ISSUED_BY; ?></option>
											<?php } ?>
								...
								<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
								  <select id="modeOfPayment" name="modeOfPayment" class="form-control" onchange="GetDataOnFilter(this.value,'<?php echo site_url()?>admin_settings/Admin_Trust_setting/get_data_on_filter')">
							...
							<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4 no-pad">
								<a style="width:24px; height:24px;margin-left:.5em;" class="pull-right img-responsive" href="<?=site_url()?>admin_settings/Admin_Trust_setting/import_setting" title="Reset"><img src="<?=site_url();?>images/refresh.svg"/></a>
	...
	<div class="modal-dialog">
		...
		<div class="modal-content" style="padding-bottom:1em;">
			...
			<div class="modal-body" id="importdet" style="overflow-y: auto;max-height: 330px;">
				<?php echo form_open_multipart(site_url().'admin_settings/Admin_Trust_setting/file_import_excel_save', array('onsubmit'=> 'return save_import()')); ?>
