	...
	<div class="container-fluid container">
		...
		<div class="row form-group">
			<div class="col-lg-10 col-md-10 col-sm-10 col-xs-8">               
				<h3><span class="icon icone-crop"></span>Edit Festival:</h3> 
			</div>
		...
		<form id ="festivalForm" action="<?php echo site_url(); ?>Shashwath/update_festival_details/" enctype="multipart/form-data" method="post" accept-charset="utf-8">
				...
				<div class="body-inner">    						
					<div class="row ">
						<div class="control-group col-md-4 col-lg-6 col-sm-4 col-xs-8">
							...
							<div class="controls" style= "padding-bottom:30px">
								<input type="text" class="form-control form_contct2" value="<?php echo $SFS_NAME; ?>" id="festivalN" placeholder="" name="festivalN"  />	
							</div>
						</div> 					
					</div>
					...
					<div class= "row " >
								...
								<select id="masaCode" style="height: 30px;">
									<?php   if(!empty($masa)) {
										foreach($masa as $row1) {
											if($SFS_MASA == $row1->MASA_NAME)
												{  ?> 
												<option value="<?php echo $row1->MASA_CODE;?>|<?php echo $row1->MASA_NAME;?>" selected="selected"> <?php  echo $row1->MASA_NAME;?></option> 
											<?php } else{ ?>
								...
								<select id="bomCode" style="height: 30px;">
									<?php   if(!empty($moon)) {
										foreach($moon as $row2) { 
											if($SFS_MOON == $row2->BOM_NAME)
												{  ?> 
												<option value="<?php echo  $row2->BOM_CODE;?>|<?php echo $row2->BOM_NAME;?>" selected="selected"> <?php  echo $row2->BOM_NAME;?></option> 
											<?php } else{ ?>
							...
							<div class="form-group">
								<select id="thithiCode" style="height: 30px;">
									...
									<?php  if(!empty($thithi[0])) { 
										...
										if(($SFS_THITHI == $row->THITHI_NAME) && ($SFS_MOON == "Shuddha"))
												{  ?> 
												<option value="<?php echo  $row->THITHI_CODE;?>|<?php echo $row->THITHI_NAME;?>" selected="selected"> <?php  echo $row->THITHI_NAME;?></option> 
											<?php } else { ?>
						...
						<div class= "col-lg-2 col-md-4 col-sm-4 col-xs-12"  id='thithiCode1Div' style="display:none;" >
							<div class="form-group">
								<select id="thithiCode1" style="height: 30px" >
									
									<?php  if(!empty($thithi[1])) { 
										foreach($thithi[1] as $row1) { 
											if(($SFS_THITHI == $row1->THITHI_NAME) && ($SFS_MOON == "Bahula"))
												{  ?> 
												<option value="<?php echo  $row1->THITHI_CODE;?>|<?php echo $row1->THITHI_NAME;?>" selected="selected"> <?php  echo $row1->THITHI_NAME;?></option> 
											<?php } else{ ?>
						...
						<input type="hidden" name="moonLoad" id="moonLoad" value="<?php echo $SFS_MOON; ?>">


						<div class="control-group col-md-6 col-lg-8 col-sm-6 col-xs-12 text-center"	>
							<br/>
							<input type="hidden" value="<?php echo $SFS_ID ?>" name="SFS_ID" />  
...
<script>
	...
	$(document).ready(function() {
		var loadMoon = document.getElementById("moonLoad").value;
	 if(loadMoon == "Bahula")
		{
	...
	$('#submited').on('click',function(e){
		...
		if($('#festivalN').val() == '' || $('#tcode').val() == ''){
		 	alert('Information','Please enter details for Festival Name and Thithi Code');
		} else 
		$('#festivalForm').submit();
	});
