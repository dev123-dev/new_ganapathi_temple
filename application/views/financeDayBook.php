...
<div class="col-lg-12 col-md-12 col-sm-8 col-xs-8 pull-right text-right" style = "padding-right:0px;padding-bottom:10px;padding-top:10px; margin-top:-4em">
	...
	<a style="text-decoration:none;cursor:pointer;pull-right;" href="<?=site_url()?>finance/dayBook" title="Refresh"><img style="width:24px; height:24px" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
</div>
				...
				<tbody>
					<?php foreach($dayBook as $result) { ?>
						<tr>
							...
							<td>
								<?php if($result->TRANSACTION_STATUS == "Cancelled") {?>
									<a style="text-decoration:line-through;cursor:pointer;color:#800000;" onClick="dayBookDetail('<?=$result->FGLH_ID; ?>','<?php echo str_replace("'","\'",$result->FGLH_NAME);?>','<?=$result->VOUCHER_NO; ?>','<?=$result->FLT_DATE; ?>')"><?php echo $result->FGLH_NAME; ?></a></td>
								<?php } else { ?>
									<a style="text-decoration:none;cursor:pointer;color:#800000;" onClick="dayBookDetail('<?=$result->FGLH_ID; ?>','<?php echo str_replace("'","\'",$result->FGLH_NAME);?>','<?=$result->VOUCHER_NO; ?>','<?=$result->FLT_DATE; ?>')"><?php echo $result->FGLH_NAME; ?></a>
								<?php } ?>
							</td>
							<td><?php echo $result->VOUCHER_TYPE; ?></td>
							<td><?php echo $result->VOUCHER_NO; ?></td>
							 <td><center><?php echo $result->FLT_DR ?></center></td>
							<td><center><?php echo $result->FLT_CR; ?></center></td>
			...
			<table class="table table-bordered table-hover">
				...
				<tbody>
					<?php foreach($accountOp as $result) { ?>
						<tr>
						<td><center><?php echo $result->FGLH_NAME ?></center></td>
...
<script>
	...
	// 	url = "<?php echo site_url(); ?>finance/day_book_on_change_date";
	...
	function get_datefield_change(date) {
		...
		// }
		url = "<?php echo site_url(); ?>finance/day_book_on_check_date";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	}
	//FOR DATEFIELD
	...
	$('#EveryRadio').on('click', function() {
		...
		url = "<?php echo site_url(); ?>finance/dayBook";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	});
	...
	function generatePDF() {
		...
		$.post(url,{'radioOptHidden':$('#radioOptHidden').val(),'dateHidden':$('#dateHidden').val(),'toDateHidden':$('#toDateHidden').val(),'fromDateHidden':$('#fromDateHidden').val(),'CommitteeId':$('#CommitteeId').val(),'voucherType':$('#voucherType').val(),'paymentType':$('#paymentType').val()}, function(data) {
			...
			if(data == 1) {
				url = "<?php echo site_url(); ?>generatePDF/create_daybookreport";
	...
	function dayBookDetail(FGLH_ID,FGLH_NAME,VOUCHER_NO,FLT_DATE){
		...
		$('#dayBookDetailForm').attr('action','<?=site_url()?>finance/dayBookDetail');
	...
	function GetSendField() {
		...
		url = "<?php echo site_url(); ?>finance/finance_day_book_excel";
		$("#tddate").attr("action",url)
		$("#tddate").submit();
	}
