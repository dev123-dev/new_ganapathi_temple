...
<?php
	...
	foreach($donation as $amt) {
		$totalDonation += $amt['ET_RECEIPT_PRICE'];
			...
			<table class="table table-bordered table-hover">
				...
				<tbody>
				   <?php foreach($seva as $result) {
						...
						echo "<td>".$result['if(ET_SO_IS_SEVA = 1, count(ET_SO_SEVA_NAME), SUM(ET_SO_QUANTITY))']."</td>";
						echo "<td>".$result['if(ET_SO_IS_SEVA = 1, SUM(ET_SO_PRICE), SUM(ET_SO_QUANTITY*ET_SO_PRICE))']."</td>";
...
<div style="clear:both;" class="container">
	...
	<div class="row form-group">							
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">	
			<span class="eventsFont2">Current MIS Event Report: <span class="samFont"><?php echo $event['ET_NAME']; ?></span></span>
			<a class="pull-right" style="margin-left:5px;" href="<?=site_url()?>Report/misReport" title="Refresh"><img style="width:24px; height:24px;" title="Refresh" src="<?=site_url();?>images/refresh.svg"/></a>
		</div>
	</div>					
	<div class="row form-group">
		<form action="<?=site_url();?>Report/misReport" id="dateChange" enctype="multipart/form-data" method="post" accept-charset="utf-8" onsubmit="return field_validation();">
			...
			<div class="multiDate">
				<div class="col-lg-2 col-md-2 col-sm-3 col-xs-5">
					<div class="input-group input-group-sm">
						<input style="border-radius:2px;" id="todayDate" name="todayDate" type="text" value="<?=@$date; ?>" class="form-control todayDate"  onchange="GetDataOnDate(this.value,'<?php echo site_url()?>Report/misReport')" placeholder="dd-mm-yyyy" readonly="readonly"/>
			...
			<div class="EveryRadio" style="display:none;">
				<div class="col-lg-2 col-md-2 col-sm-3 col-xs-6">
					<div class="form-group">
						<div class="input-group input-group-sm">
							<input name="fromDate" onchange="GetDataOnDate(this.value,'<?php echo site_url()?>Report/misReport')" id="fromDate" type="text" class="form-control fromDate2" value="<?=@$fromDate; ?>" placeholder="From: dd-mm-yyyy" readonly="readonly"/>
							<div class="input-group-btn">
				...
				<div class="col-lg-2 col-md-2 col-sm-3 col-xs-6">
					<div class="form-group">
						<div class="input-group input-group-sm">
							<input name="toDate" onchange="GetDataOnDate(this.value,'<?php echo site_url()?>Report/misReport')" id="toDate" type="text" value="<?=@$toDate; ?>" class="form-control toDate2" placeholder="To: dd-mm-yyyy" readonly="readonly"/>
							<div class="input-group-btn">
		...
		<?php 
			...
			foreach($eventReceiptCategory as $result) { ?>
				<a href="#" onclick="openCity(event, '<?=$i++;?>');">
					<div id="<?=$j--;?>" class="w3-third tablink w3-bottombar w3-hover-light-grey w3-padding"><?=$result['ET_RECEIPT_CATEGORY_TYPE'];?></div>
				</a>
		<?php } ?>
	</div>
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				...
				<table class="table table-bordered table-hover">
					...
					<tbody>
						<?php foreach($seva as $result) {
							...
							// Lathish code Start
							echo "<td>".$result['POSTAGE_PRICE']."</td>";
							echo "<td>".($result['if(ET_SO_IS_SEVA = 1, SUM(ET_SO_PRICE), SUM(ET_SO_QUANTITY*ET_SO_PRICE))'] + $result['POSTAGE_PRICE'])."</td>";
							// Lathish code Ends
							echo "</tr>";
						} ?>
					</tbody>
				...
				<table class="table table-bordered table-hover">
					...
					<tbody>
						<?php foreach($donation_details as $result) {
							echo "<tr>";
							echo "<td>".$result['ET_RECEIPT_NO']."</td>";
							echo "<td>".$result['ET_RECEIPT_NAME']."</td>";
							echo "<td>".$result['ET_RECEIPT_ADDRESS']."</td>";
							echo "<td>".$result['ET_RECEIPT_PHONE']."</td>";
							echo "<td>".$result['ET_RECEIPT_PRICE']."</td>";
							echo "<td>".$result['ET_RECEIPT_PAYMENT_METHOD_NOTES']."</td>";
							echo "</tr>";
						} ?>
					</tbody>
				</table>
				...
				<table class="table table-bordered table-hover">
					...
					<tbody>
						<?php foreach($hundi as $result) {
							echo "<tr>";
							echo "<td>".$result['ET_RECEIPT_NO']."</td>";
							echo "<td>".$result['RECEIPT_ET_NAME']."</td>";
							echo "<td>".$result['ET_RECEIPT_PRICE']."</td>";
							echo "<td>".$result['ET_RECEIPT_PAYMENT_METHOD_NOTES']."</td>";
							echo "</tr>";
						} ?>
...
<script>
	...
	function GetSendField() {
		...
		url = "<?php echo site_url(); ?>Report/event_mis_report_excel";
	...
	function generatePDF() {
		...
		let url = "<?php echo site_url(); ?>generatePDF/create_misReceiptSessionPrint";
		$.post(url,{'radioOptHidden':$('#radioOptHidden').val(),'dateHidden':$('#dateHidden').val(),'toDateHidden':$('#toDateHidden').val(),'fromDateHidden':$('#fromDateHidden').val()}, function(data) {
			if(data == 1) {
				url = "<?php echo site_url(); ?>generatePDF/create_misReceipt";
	...
	function printPage() {
		...
		$.post(url,{'radioOptHidden':$('#radioOptHidden').val(),'dateHidden':$('#dateHidden').val(),'toDateHidden':$('#toDateHidden').val(),'fromDateHidden':$('#fromDateHidden').val()}, function(data) {
			if(data == 1) {
				let url2 = "<?php echo site_url(); ?>generatePDF/create_misReceipt";
				var win = window.open(
					url2,
					'_blank');
