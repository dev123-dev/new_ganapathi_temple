...
$("#rashi").on("keyup",function(e){
    ...
    if(e.keyCode == 13){
		...
		if($("#rashi").val() != "") {
			...
			var response = $.ajax({
				type: "POST",
				url: base_url+"TrustReceipt/getRashi",
				data: {
					keyword: $("#rashi").val()
				},
				dataType: "json",
				success: function (data) {
					...
					for(let i = 0; i < data.length; ++i) {
						if (data.length >= 0) {
							$('#DropdownRashi').append('<li id="'+ data[i]['RASHI_ID'] +'" role="presentation" onClick="getNakshatra('+ data[i]['RASHI_ID'] +')" class="newli"><a>' + data[i]['RASHI_NAME'] + '</a></li>');
...
$("#nakshatra").on("keyup", function(e){
    ...
    if(e.keyCode == 13){
		...
		if($("#nakshatra").val() != "") {
			...
			var response = $.ajax({
				type: "POST",
				url: base_url+"/TrustReceipt/getNakshatra",
				data: {
